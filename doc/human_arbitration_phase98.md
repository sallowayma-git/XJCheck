...
## Phase 180：端子续表结构回放（2026-07-18）

- 本轮没有新增端口内部联通裁决，沿用已确认的端子合同：实例名/表头与行号组合成独立逻辑端口，每个端口只向同一行外部端点映射，端口之间不联通、不建立 electrical union。
- `上接<prefix><N> + 说明` 是既有三列表端子的续表结构证据。只接受从 `N+1` 开始的至少两行连续编号，并记录完整文本 ID；示例包括 `1-21GD-19 -> 1-21DK2-2`、`4Q1D-38 -> 4n431`、`1YD-49 -> 1n520`。空行不建立映射。
- 普通表头与续表头的所有权必须隔离；本轮 full replay 曾发现续表头抢占普通端点并隐藏真实 many-to-one，修复后 `1QD-2 -> 1KLP1-1`、`3-2QD-7/8 -> KD26` 及其审核项均恢复。
- 28 项目 / 533 DWG final fresh3 新增 1212 条续表 mapping，全部具备 `0.95/pass`、`说明`、连续行和完整表头/行号/端点文本证据，旧 mapping 移除 0。
- 端子表文本 `GND` 的多端口共用语义尚待人工裁决。当前保留 8000/9000、30000、PAC 的相关 many-to-one review；在确认前不得按名称或通用接地符号规则静默。

## Phase 180：背板实例作用域与标题权威（2026-07-18）

- 背板表格端口必须组合本页真实装置实例。明确包含 `REAR WIRING/背板` 的 CAD 标题拥有实例权威，优先于整页自由文本和标题栏中重复出现的其他装置名；例如 WBH 四页分别使用 `1n / 1-2n / 3-2n / 5n`，不得统一误写成 `1-1n / 1-20n`。
- 短式实例 `1n/2n/5n` 只在明确 rear-wiring 标题上下文中生效。无该上下文的全页自由文本继续要求既有连字符实例语法（如 `1-20n`）；裸 `1n/4n/5n`、文件名片段或最高 Y 文字均不能单独授予作用域。
