...
def test_write_audit_outputs_emits_issue_artifacts_with_evidence_fields(tmp_path: Path) -> None:
    ...
    project_dir = write_project_artifacts(ProjectArtifacts(scan=scan, pairs=[pair]), tmp_path)
    ...
    assert findings_payload["pair_evidence_summary"]["pairs_with_evidence"] == 1
    ...
    assert "04.dwg" in report_text
    assert "sheet_no=04" in report_text
    assert "sheet_order=4" in report_text
    assert "line_group=G0001" in report_text
    assert "line_start=[10.0, 20.0]" in report_text
