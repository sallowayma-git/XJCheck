...
def test_extract_strip_two_port_component_pairs_accepts_boundary_ampersand_singletons() -> None:
    sheet = _make_sheet()
    texts = [
        _make_text("BODY", "1CLP4", 205.0, 205.3, layer="MARK"),
        _make_text("PORT1", "1", 208.9, 190.0, layer="0", source_block_name="FJL-25-2A_Mirror"),
        _make_text("PORT2", "2", 208.9, 175.0, layer="0", source_block_name="FJL-25-2A_Mirror"),
        _make_text("TOP", "1KD5 &", 204.3, 195.4),
        _make_text("BOTTOM", "1n922 &", 205.8, 169.8),
    ]
    ...
    assert {pair.evidence["external_endpoint_raw"] for pair in pairs} == {"1KD5 &", "1n922 &"}


def test_extract_strip_two_port_component_pairs_accepts_boundary_ampersand_comma_list() -> None:
    sheet = _make_sheet()
    texts = [
        _make_text("BODY", "5KLP10", 205.0, 205.3, layer="MARK"),
        _make_text("PORT1", "1", 208.9, 190.0, layer="0", source_block_name="FJL-25-2A_Mirror"),
        _make_text("PORT2", "2", 208.9, 175.0, layer="0", source_block_name="FJL-25-2A_Mirror"),
        _make_text("TOP", "5KLP3-1,5KLP2-1 &", 204.3, 195.4),
        _make_text("BOTTOM", "5n112", 205.8, 169.8),
    ]
    ...
    assert {pair.evidence["external_endpoint_raw"] for pair in pairs if pair.right_text_id == "TOP"} == {
        "5KLP3-1,5KLP2-1 &"
    }


def test_extract_strip_two_port_component_pairs_rejects_interior_or_isolated_ampersand() -> None:
    sheet = _make_sheet()
    base = [
        _make_text("BODY", "1CLP4", 205.0, 205.3, layer="MARK"),
        _make_text("PORT1", "1", 208.9, 190.0, layer="0", source_block_name="FJL-25-2A_Mirror"),
