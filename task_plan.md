...
- Loop state machine (persist after every transition): `DISCOVER_CORPUS` -> `BASELINE_REPLAY` -> `VISUAL_REVIEW_QUEUE` -> `EVIDENCE_CLUSTER` -> `PARALLEL_PROBES` -> `SELECT_ONE_SLICE` -> `IMPLEMENT` -> `FOCUSED_TESTS` -> `BOUNDED_REPLAY` -> `IDENTITY_AUDIT` -> (`REJECT_AND_RECORD` or `ACCEPT_SLICE`) -> `REGRESSION_REPLAY` -> `VISUAL_RECHECK` -> `NEXT_CLUSTER`. If no evidence-backed slice exists, record `NO_SAFE_CHANGE`, preserve fail-closed behavior, and continue discovery rather than stopping.
- Every cycle starts by running the current engine on the target drawing set (first as a bounded leaf `.prj`, then affected project, then the full `正式典设` corpus only after repeated bounded acceptance). For each page, inspect the generated numbered-group/Pair output against the original drawing using visual inspection; queue every missing, merged, split, or semantically wrong group with source page, region, expected reading, actual reading, and artifact IDs.
- Visual review protocol: use rendered page images/crops when available; otherwise generate a page crop from the authoritative source/converted DXF. Review the exact region containing the number group, not only report summaries. Record `expected_groups`, `recognized_groups`, `false_groups`, `ambiguous_groups`, and a `visual_evidence_ref` in findings. A visual observation is a lead until corroborated by raw CAD entities, geometry, block/XDATA, or a reproducible extractor artifact.
- Evidence clustering: normalize issues by producer layer (conversion/parser), text/tokenization, geometry/group ownership, candidate scoring, pairing/cardinality, page scope, and audit rule. Cluster by structure and evidence, never by project name, filename, fixed handle, fixed value, page index, or issue count. Select exactly one smallest cluster per cycle with at least one positive and one adversarial negative fixture.
- Parallel probe policy: dispatch independent, read-only probes for corpus inventory, raw geometry/visual evidence, candidate/pair pipeline, and negative-corpus impact. Require exact `file:line`, project/page, Pair ID, and artifact paths. Stop silent probes at the ten-minute threshold, close/ignore incomplete results, and never substitute weaker evidence or another model.
- Parallel probe wait override (2026-08-19): the user explicitly cancelled the ten-minute subagent limit. Continue waiting for probes to complete regardless of duration unless they explicitly fail or cannot make progress. This latest instruction supersedes earlier timeout language in the goal contract and historical cycle notes.
- Implementation policy: main thread reads the exact code to be modified; add a narrow, name-independent contract plus focused positive/negative tests. Do not patch audit rules to hide producer omissions. Any multi-endpoint structure must remain membership-only unless the source proves canonical endpoint, pairwise relations, connectivity, and union semantics.
- Acceptance gates per slice: focused tests pass; `compileall` and `git diff --check` pass; bounded raw replay is complete; Pair IDs and all non-evidence fields are identity-equal except explicitly expected changes; audit JSON/Parquet/Markdown counts agree; issue additions are zero unless explicitly justified; removals are matched by exact issue identity; visual recheck confirms the intended group and all selected negatives remain correct.
- Regression cadence: after each accepted bounded slice, replay every affected leaf project, then the complete 1,212-DWG `正式典设` corpus in isolated project processes when feasible, and finally the existing 28-project/533-DWG `test/` control. Keep each fresh/audit bundle in a new `.tmp/phase195_*` directory; never overwrite historical evidence.
- Persistence protocol: update `task_plan.md` at state transitions, append `findings.md` after every discovery/decision/error, and append `progress.md` after every command batch, replay, visual review, and gate. Retain a machine-readable cycle ledger with baseline artifact, selected cluster, code/test files, replay roots, identity deltas, visual evidence references, and disposition.
- Stop/pause semantics: the loop stops only on explicit user pause/stop, a safety blocker requiring user authority, or an unrecoverable environment failure documented three times. A passing cycle, a no-change cycle, a subagent timeout, or a clean audit is not permission to stop. Do not push to a remote or alter protected/unrelated files without explicit authorization.
- Guardrails: no filename/project/handle/value shortcuts; no issue-count-only acceptance; no pairwise union for multi-endpoint networks; incomplete evidence remains `review`/fail-closed; root `package-lock.json`, `.planning/*`, and unrelated desktop/preview work stay out of recognition commits.
- **Current loop state:** cycles 195-01 through 195-05 are accepted; Cycles 195-06 through 195-09 are persisted as `NO_SAFE_CHANGE`. Current authority remains `.tmp/phase195_formal_full_fresh4` plus `.tmp/phase195_formal_full_audit5`: 49/49 projects, 1,212/1,212 DWG, 88,265 Pairs, zero incomplete, and 297 issues. Cycle 195-10 is in `EVIDENCE_CLUSTER` over the five remaining p017/p021 low-confidence edge rows. Join each issue to Pair/candidate/raw geometry and never solve by lowering the audit threshold.
- **Cycle 195-08 terminal subcluster disposition:** six p010/p011/p015 ordinary terminal-grid reviews were joined to `pairs.parquet` and the corresponding table summaries. All target text IDs have zero table-mapping ownership matches and no generated table Pair; neighboring mappings are different rows/columns. The evidence is insufficient for a producer-owned shadow or mapping contract, so this subcluster is `NO_SAFE_CHANGE` and remains fail-closed. The next active probe is the p016-p021 component-grid family.
- **Cycle 195-08 p003 component subcluster disposition:** `PC0029`, `PC0031`, `PC0033`, and `PC0037` are internal virtual-pin boundary mispairs (`1 -> 401/402/405/818`) from Y4/R4 mirror block variants. Extending only the candidate guard would hide producer-owned endpoints, while extending the producer without duplicate-anchor disambiguation is not fail-closed. The p003 rules replay is byte-identical to formal audit5; disposition is `NO_SAFE_CHANGE` and the rows remain fail-closed.
- **Cycle 195-08 dominant component-grid disposition:** the 207 p016-p021 rows are visually confirmed TSB-14 rounded grid/frame borders (`105 x 2->28`, `102 x four-digit->27`), not conductors. However the persisted producer evidence exposes only the definition name, internal virtual segment number and dimensions; a proposed shadow would therefore depend on `TSB-14`, `:VIRTUAL:56` and exact 70-unit geometry rather than a name-independent closed-grid ownership contract. The draft was fully removed. Six-project rules replay is 24/24 audit artifacts byte-identical to formal audit5. Cycle disposition is `NO_SAFE_CHANGE`.
- **Current loop state:** Cycles 195-09 through 195-12 are persisted without source/test changes. Their bounded audits are byte-identical to audit5 for p039-p044 (24/24 files), p017/p021 (8/8 files), p008/p009/p012/p014/p020 (20/20 files), and the seven many-to-one projects p006/p014/p015/p021/p038/p039/p040 (28/28 files). Cycle 195-12 retained all 13 formal `R-MANY-TO-ONE` reviews under their producer/cardinality contracts; the next active state is corpus-wide discovery of unrecognized or under-recognized numbered groups.
- **Cycle 195-06 recoverable errors:** the raw/visual and corpus-census probes exceeded ten minutes without usable output and were interrupted; a bundled `rg.exe` helper search failed to start with Windows `Access denied`; a follow-up search assumed a nonexistent repository `scripts/` directory. All were read-only and changed no source or artifacts. Continue with exact CLI entrypoints and PowerShell-native discovery; do not treat silent probes as evidence or repeat either failed search form.
- **Cycle 195-04 recoverable error:** the first three broad read-only cluster probes (many-to-one, missing-side, low-confidence) all returned the same provider-boundary HTTP 504 after waiting and produced no evidence or file changes. Do not treat them as audits. The recovery path is a main-thread issue-only structural aggregation followed by new short probes scoped to exact project/Pair IDs; do not repeat three broad corpus scans.
- **Accepted slice ledger (cycle 195-01):** selected cluster `component_diagrams/strip_endpoint_boundary_ampersand`; implementation `src/dwg_audit/audit/component_diagrams.py:963-968`; tests `tests/unit/test_component_diagrams.py:327-389`; focused/full-unit gates `30/30`, `1333 passed, 1 skipped`; bounded Pair identity `1063 -> 1091` with 28 additions, zero deletions, and only the two expected missing-side rows changing `review -> discard`; audit issue identities `3 -> 0` with no additions, including the structurally justified existing comma-group closure removal; visual refs `...phase195_formal_gj73_single_visual2/crops/PCM0027.png`, `PCM0034.png`, `PCM0002.png`, `PCM0045.png`, `PCK0001.png`; disposition `ACCEPT_SLICE`.
- **Accepted slice ledger (cycle 195-02):** selected cluster `rules/schematic_small_port_duplicate`; implementation `src/dwg_audit/audit/rules.py`; tests `tests/unit/test_pairs_and_rules.py`; gates `172 passed`, full unit `1339 passed, 1 skipped`, compileall/diff-check clean; bounded p003 Pair identity 523/523 exact, audit `5 -> 4` with sole removal `R-DUPLICATE-PAIR/PCS0001/PAB0001`; current full formal audit `334 -> 333`, additions zero; 533-control audit `9 -> 9`, identity exact; visual refs `.tmp/phase195_formal_dup_p003_visual/crops/PCS0001.png` and `PAB0001.png`; disposition `ACCEPT_SLICE`.
- **Accepted slice ledger (cycle 195-03):** selected cluster `rules/same_sheet_kk_small_port_alias`; evidence is p040/S0018/F0018 `PCK0017+PCS0001 (JD8)` and `PCK0018+PCS0003 (JD3)`. Raw/corpus/rule probes found only these two cross-submode overlaps in 88,265 Pair facts; both share `B1022/KK2P`, port/endpoint text IDs and coordinates, `GC0042`, and `L4061/L4063`, with hidden `2-1n` versus visible `KZKK` body labels. Implementation is `src/dwg_audit/audit/rules.py:1320-1326,1455-1638`; tests are `tests/unit/test_pairs_and_rules.py` same-sheet alias fixtures. Gates: focused rules `182 passed`, full unit `1380 passed, 1 skipped`, compileall/diff-check clean. Bounded Pair rows `1800 -> 1800`, all fields exact; issue identity `11 -> 9`, only the two target removals. Full formal audit `.tmp/phase195_formal_full_audit3` is `333 -> 331`, zero additions and zero retained-object drift; 533-control `.tmp/phase195_control_533_audit2` is identity-exact `9 -> 9`. Visual refs `.tmp/phase195_formal_p040_visual3/crops/PCK0017.png` and `PCK0018.png`; disposition `ACCEPT_SLICE`.
- **Accepted slice ledger (cycle 195-04):** selected cluster `candidates/schematic_ud_numeric_peer`; implementation `src/dwg_audit/audit/candidates.py`; tests `tests/unit/test_terminal_candidates.py`; final gate `1383 passed, 1 skipped`, compileall/diff-check clean. Full formal fresh4 is 49/49, 1,212 valid, 88,265 Pairs and exactly 32 intended semantic changes; audit4 is `331 -> 299`, additions zero and 299 retained objects exact. Fresh 533-control is 28/28, 533 valid, no accepted UD, and audit identity-exact `9 -> 9`. Visual refs `.tmp/phase195_c04_ud_visual_baseline/p041_S0006_UD_expected.png` and `.tmp/phase195_c04_ud_p041_visual/p041_S0006_UD_current.png`; disposition `ACCEPT_SLICE`. Machine-readable record: `phase195_cycle_ledger.json`.
- **Cycle 195-05 evidence contract:** raw DXF proves `PCB0001 3-2ZK-4 -> 3-2n221` and `PCB0002 1-2ZK-4 -> 1-2n221` are vertical `FJL-25-2A_Mirror` two-port bridges whose shared endpoints are independently described by p039 backplate rows `P0282` and `P0179`. Corpus census found seven `strip_two_port_endpoint_bridge` facts in p006/p039/p047/p048; all seven have exact endpoint text/coordinate/line-group/raw-value ordinary shadows, but only the two p039 facts share a right endpoint with a complete cross-file backplate table row. Five p039 near-geometry ordinary negatives have confidence `0.7754..0.804`, no component identity and different endpoint/text/group evidence. The rule must require exactly one bridge and one table Pair, unique IDs, pass/native finite confidence >=0.95, distinct nonempty sheets/files, exact shared right value, complete top/bottom grammar and identity, complete line/score evidence, and a producer-consistent pluginless backplate row tied to the same device prefix. Any extra Pair or malformed/missing/conflicting field stays visible.
- **Accepted slice ledger (cycle 195-05):** selected cluster `rules/strip_two_port_endpoint_bridge_backplate_chain`; implementation `src/dwg_audit/audit/rules.py` plus Pair score fidelity in `src/dwg_audit/report/rerun.py`; tests `tests/unit/test_pairs_and_rules.py` and `tests/unit/test_rerun_audit.py`. Bounded p039 fresh is 30/30 valid and 2,635/2,635 Pair objects exact; accepted audit is `.tmp/phase195_c05_p039_audit3`, `15 -> 13`, zero additions, exact target removals `PCB0001/P0282` and `PCB0002/P0179`, and zero retained-object drift after excluding sequence-derived `issue_id`. Visual refs are the three black-background crops under `.tmp/phase195_c05_p039_visual2`. Formal audit5 is `299 -> 297`, only the same two removals across 49 projects; 533-control audit2 is identity/object exact `9 -> 9`. Final gates pass `1387 passed, 1 skipped`, compileall and diff-check; disposition `ACCEPT_SLICE`.
- **Status:** in_progress
...
## Phase 195 cycle 195-13 closure and cycle 195-14 handoff

- Cycle 195-13 selected `discovery/unrecognized_number_group_census/orphan_numeric_text_vs_component_port_evidence` and completed the full discovery, evidence clustering, positive/adversarial visual review, producer-contract audit and bounded replay gates.
- The loop must not turn every unreferenced numeric TEXT into a numbered group. In the accepted p039 positive, internal ports `1/2` are evidence members of a producer-owned bridge whose Pair values are external `3-2ZK-4 -> 3-2n221`; the same geometry without endpoints is a natural negative. Keep candidate rejection and component evidence-only semantics fail-closed.
- Cycle 195-13 is persisted as `NO_SAFE_CHANGE` in `phase195_cycle_ledger.json`. Current authority remains `.tmp/phase195_formal_full_fresh4` / `.tmp/phase195_formal_full_audit5`, with 297 issues (`261` low-confidence, `18` missing-side, `13` many-to-one, `5` cross-page). No implementation file or test file was changed.
- Cycle 195-14 is now active in `EVIDENCE_CLUSTER` on the repeated p012/p013 `grid_heavy_wire_diagram` family where alphanumeric designators are rejected as nonnumeric and produce repeated `? -> numeric` review rows. Next actions: independently join exact Pair/text/line evidence, inspect representative original-DXF regions and natural negatives, then either implement a producer-owned grammar with adversarial coverage or record another `NO_SAFE_CHANGE` and continue discovery.

## Phase 195 cycle 195-14 implementation (2026-08-30, user-directed TSB terminal-strip slice)

- User took over the session and redirected cycle 195-14 to the p016-p021 TSB terminal-strip family with two hard requirements: run the original drawings through a live routing test before coding, and make the recognition structural so renamed blocks (CSB/ASB-like) are recognized without name hard-coding.
- Live route test on `26 元件接线图4.dwg` (p020/F0026) confirmed the pre-change failure: route `元件接线图/horizontal_component/ComponentDiagramExtractor` (0.82) with zero component-mapping output and 32/32 wrong ordinary reviews (`2->28`, `xxxx->27`) from the strip border verticals. The existing route already had a strip layer (FJL-25-2A two-port + ZK-n bridge) but no producer recognized terminal-strip lattices.
- Accepted contract: `extract_terminal_strip_lattice_pairs` detects strips structurally (two-column numeric pin lattice with uniform 2.5-8.0 pitch, pins owned by any block, instance label `^[A-Za-z]{1,6}\d*$` 0.5-4.0 pitches above the top pin, flanking vertical border groups covering the lattice with >=1.0 margin). Rows pair left/right labels by y (tol 1.5); both cleaned values must match the existing external-endpoint grammar; unlabeled rows and single-sided rows emit nothing; flanking border groups are consumed so the wrong ordinary rows discard. Block names are evidence-only.
- Impact (bounded replay, all six TSB projects): 214 -> 42 issues; 210 wrong reviews eliminated; 418 pass mappings emitted and value-verified against the user-confirmed reading (incl. `1I6D7 -> 1I6D4` cross-strip device rows). Corpus census: detector fires on exactly the 11 TSB sheets, zero other projects. Gates: `1396 passed, 1 skipped`, compileall clean.
