...
- Audit11 replays the hardened authoritative contract with exactly the same 55 issue identities as audit10. A Sol independent review found that `source_block_name` must be casefolded before scope grouping; otherwise `BLOCK` and `block` can masquerade as distinct explicit scopes. The same review found that the reused-header fixture lacked `table_mapping.sheet_id` and therefore did not exercise the target branch. Both gaps are now covered by direct negative tests; the complete rules gate is `109 passed`.
- PAC `14 交流电压回路2.dwg` needs no new generic endpoint relaxation. Fresh10 already emits `1F1-1 -> 1VD1` and `1F1-2 -> 1701` as `component_mapping/pass/0.97` through the insert-backed two-port geometry model, while the ordinary `? -> 1701` observation remains shadow-only.
- The 31000 `1731/1729` residual family consists of three standalone `CONNECT` lines plus nearby layer-0 closed LWPOLYLINE top edges claiming the same text. Any accepted geometry change may shadow only the proven enclosure-edge Pair while retaining the CONNECT Pair and its missing-side review unless independent endpoint authority is found.
- Performance audit remains a separate slice: production extraction retains multiple full-project representations, preview loads whole-project parquet before filtering, and the inner `ezdxf.odafc` subprocess capture is outside the existing bounded worker tail. These risks are recorded for a dedicated 40-DWG/RSS loop and are not part of the current recognition commit.
- The first post-replay issue comparison assumed `issues.json` was an object; the actual artifact may be a top-level list. The failed read produced no artifact or code change. The corrected reader accepts both shapes and confirms 31000 audit identity is unchanged.
- The accepted non-tall enclosure thresholds are geometry-relative: intersection covers at least 95% of the shorter horizontal span and the two spans are at least 75% of each other. This covers the observed 62.5/80 and 247.5/265.21 cases while the direct short-line negative remains fail-closed.
- Independent code review found four unsafe proof gaps and all were closed before final replay: canonical groups now require only `LINE/CONNECT` members, alternative or ambiguity-bearing Pairs cannot prove uniqueness, nullable text-id sentinels are rejected, and a canonical line may not extend beyond the frame edge. Ten focused geometry tests cover the accepted and rejected paths.
- Independent corpus review confirmed the two additional 26000 hits are the same geometry family: `PW0150 -> PW0148` for text `229` and `PW0151 -> PW0149` for `431`. Both parents are geometrically closed four-edge rectangles despite DXF closed bit 0, and both canonical lines are independent `LINE/CONNECT` entities.
- Final fresh13/audit13 acceptance: 28 projects, 533 files/sheets, 533 valid, zero invalid/incomplete; 38,814 Pair identities and 55 issue identities have zero additions/removals versus fresh10/audit11. The five auxiliary-frame shadows are exactly the two confirmed 26000 rows and three target 31000 rows.

## Phase 182 residual clustering

- All three `R-CROSS-PAGE-CONFLICT` rows remain evidence-backed review: 24000 and 25000 reuse `NCK305` without a device/plugin instance authority; PAC reuses the same `1n/NCK305` instance and row across pages, which is stronger conflict evidence. Source block names, page coordinates and semantic notes do not prove distinct physical scopes.
- The first six-way broad Sol scan returned only the cross-page audit; five agents remained silent past ten minutes and were closed. Subsequent probes must be bounded to exact projects and Pair IDs rather than scanning all 38,814 Pair facts.
- The 19 missing-side rows split into known fail-closed families: 8000/9000 textless `1201/1204`, 31000 `926/927` and retained CONNECT claims, 23000/26000 standalone numeric rows, and four PAC rows. The two low-confidence rows are 23000 `PW0170 1309 -> 10` and 31000 `PW0225 932 -> 932`.
- Repeated many-to-one families are now explicit: CLP9/backplate structured sharing across 18000/19000/20000/2_2, terminal-header/component shared endpoints across seven projects, and PAC/WBH device-specific structured sharing. These require exact topology/ownership review before any cardinality exemption.
- The initial fifteen-review safe-contract hypothesis was disproved by bounded evidence: the six KLP1 terminal-header/component groups have authoritative same-sheet continuation chains and remain fail-closed. Only the complete backplate virtual-row bridges qualify.
- The contract must not depend on KLP/CLP names, `n425/n427`, row 25, project names or handles. It requires complete component body/port/support-line/text evidence, an authoritative table row with valid sequence and column roles, disjoint ownership, and no component chain starting from the shared endpoint.
- 23000 `PW0170` is a separate block-port extraction gap; `PW0493` plus 26000 `PW0117/PW0127/PW0148/PW0149` are multi-drop bus/net-label ordinary-pair noise; 23000 `PW0519` and PAC `PW0146/PW0148` are auxiliary-frame/annotation geometry. PAC `PW0147` is a range-endpoint extraction gap (`1QD47~53`), while 31000 `PW0225 932->932` must remain low-confidence review.

## Phase 182 backplate virtual endpoint contract correction

- Four bounded `gpt-5.6-sol` audits corrected the initial 15-row hypothesis. The six terminal-header/KLP groups all have authoritative same-sheet component chains starting from the shared endpoint, so they remain review. The safe family is the complete backplate virtual-row bridge only.
- The safe family contains ten natural audit13 rows, not nine: the expected nine CLP9 rows in 18000/19000/20000/2_2 plus 23000 `PCM0073`, a generic `1-4CLP2-2 -> 1-4n611` component mapping paired with the complete independent virtual row `1-4n511 -> 1-4n611`.
- The generalized proof is name-independent. It requires exactly one authoritative component mapping and one authoritative table mapping on distinct sheets/files; both pass at `>=0.95`; the device instance plus plugin slot must reproduce the header prefix; raw row, middle value and logical endpoint must agree; raw header and plugin title must agree; semantic notes and virtual/external column roles must be present. Incomplete evidence and all same-sheet component continuations remain visible.
- Full original-DWG fresh replay and audit acceptance completed in `.tmp/phase182_full_533_fresh` and `.tmp/phase182_full_533_audit2`: 28 projects, 533/533 valid pages, zero invalid/incomplete, 38,814 Pair identities, and issue identities `55 -> 45`. Exactly the ten expected reviews were removed and no issue was added; the resulting distribution is many-to-one `21`, missing-side `19`, cross-page `3`, low-confidence `2`. The remaining 45 issues are next-round fail-closed audit objects, not a clean corpus.

## Phase 183 recovery and residual candidate audit (2026-07-20)

- Recovered `master` at `519e6d7 Recognize comma-grouped strip component endpoints`; current tracked dirty state is only a nine-line deletion of `_is_authoritative_comma_component_mapping_pair()` in `rules.py`. Full-repository reference search proves the helper had no caller. `_is_authoritative_comma_component_mapping_group()` independently retains the active comma contract, so the deletion is behavior-neutral inside the repository.
- Root `package-lock.json` remains untracked, untouched, and outside the recognition boundary.
- `.tmp/phase183_dyqk_baseline_current_rules_audit` is a complete rules-only audit artifact for 23000: six JSON/Parquet/Markdown issue identities, distribution `many-to-one 3 / missing-side 2 / low-confidence 1`, exact against `.tmp/phase182_full_533_audit2/23000_A`. Its extraction provenance comes from the authoritative Phase182 33/33 complete bundle rather than a local manifest in the rules-only directory.
- The committed comma-grouped implementation already removes nine complete terminal-header/component reviews in the existing focused artifact, including 23000 `PCM0044`. It must not be widened to any raw endpoint containing a comma: WBH `PCM0101` lacks the complete terminal-header graph and remains a required negative.
- The strongest next residual candidate is `PAC-885G-H / 15 交直流回路.dwg / PW0147 / R-PAIR-MISSING-SIDE / ? -> 431`. Its missing endpoint is occupied by unblocked text `1QD47~53` within about one drawing unit of a short horizontal `LINE/CONNECT` endpoint; the opposite `431` candidate is strong and uniquely selected.
- Proposed semantic boundary: recognize a well-formed, ascending terminal range label only as endpoint-local schematic semantic evidence. Emit `semantic_mapping/review` and make the shadow ordinary half-pair ineligible; do not split or expand the range, infer internal connectivity, or claim electrical union. A same-side competing endpoint label, including the existing strict-geometry negative fixture, must reject the semantic takeover.
- The first combined planning patch guessed a Phase182 heading that exists only in `progress.md`, so `apply_patch` rejected the whole batch before changing any file. The updates were split against each file's actual tail. The first resumed subagent name also collided with a stale task path before creation; a new clean-context `gpt-5.6-sol` task name completed the same read-only audit.
- A local read-only census over all 28 Phase182 extraction bundles found 20 `^\d+QD\d+~\d+$` texts. Only PAC `S0015/GW0147/T0998` satisfies the complete proposed contract. Every other natural occurrence is excluded by grid orientation, line length above 50, non-CONNECT/mixed layer ownership, no strong opposite numeric endpoint, same-side competition, or endpoint distance.
- The closest natural negative is PAC `GW0150`: it shares `T0998`, a 32.5-unit pure CONNECT horizontal group and a strong opposite `114`, but the label is 9.202 units off-row. A 4-unit endpoint-local x/y bound separates it from `GW0147` (`0.6635/0.798`) without using page or terminal values.
- The all-corpus census subagent remained silent beyond the ten-minute limit and was interrupted. Its unfinished result is not evidence; the main-thread Parquet census completed in 1.3 seconds and produced the counts above without writing artifacts.
- The first independent code-review probe used the required clean-context `gpt-5.6-sol` route but failed at the service boundary with `bad_response_status_code`; it returned no review evidence and changed no file.
- Concurrent `.planning/perf_phase3b1_fix/*` modifications appeared after the Phase183 edits. They are outside the recognition scope and must remain unstaged and untouched together with root `package-lock.json`.
- Focused candidate coverage passes `64 passed`. Raw PAC replay `.tmp/phase183_terminal_range_pac_fresh` is complete at 31/31 valid DWG/sheets with zero invalid/incomplete; current audit is `.tmp/phase183_terminal_range_pac_audit`.
- PAC Pair comparison is `2012 -> 2012` with no Pair ID additions/removals and exactly one semantic row change: `PW0147/GW0147` changes from `? -> 431 / ordinary_pair / confidence .535` to `1QD47~53 -> 431 / semantic_mapping / confidence .985`. Its evidence carries `schematic_terminal_range_label`, `ordinary_pair_eligible=false`, and no internal-connectivity or electrical-union claim.
- PAC issue comparison is `12 -> 11`, with no additions and exactly one removal: `R-PAIR-MISSING-SIDE / S0015 / PW0147 / ? -> 431`. Rule distribution changes only missing-side `4 -> 3`; many-to-one `7`, cross-page `1` remain unchanged.
- Current PAC candidate artifact has exactly one accepted terminal-range candidate (`GW0147/T0998`) and three rejected same-text natural neighbors (`GW0149` twice and `GW0150`) for out-of-scope geometry, including the 9.202-unit off-row `GW0150` negative.
- Final gates pass: candidate/rules bundle `177 passed`; full repository `1157 passed, 1 skipped`; `python -m compileall -q src tests`; global `git diff --check`. The first full-test process completed after its exec handle was lost during a context transition, so it was not counted; the explicit rerun produced the authoritative result above.
- Final scoped commit set is exactly `findings.md`, `progress.md`, `task_plan.md`, `src/dwg_audit/audit/candidates.py`, `src/dwg_audit/audit/rules.py`, and `tests/unit/test_terminal_candidates.py`. Concurrent `.planning/perf_phase3b1_fix/*`, desktop/sidecar files and root `package-lock.json` remain excluded.
- Existing Phase183 dyqk artifacts predate this implementation and are preserved. The only new artifact pair is the authoritative bounded acceptance `.tmp/phase183_terminal_range_pac_fresh` plus `.tmp/phase183_terminal_range_pac_audit`; there is no new superseded process artifact to remove.
- Published `994d0e7 Generalize terminal range endpoint semantics` to `origin/master`; it contains exactly the six files listed above. The active residual goal continues, and the next loop must establish a full current issue identity set rather than subtract focused removals from the Phase182 total.

## Phase 184 current-corpus baseline recovery (2026-07-20)

- `HEAD=5e9509a` is synchronized with `origin/master`; recognition files are clean. Unstaged changes are concurrent `.planning/perf_phase3b1_fix/*`, desktop/sidecar files, packaging tests, and root `package-lock.json`; none is in scope.
- Phase182 fresh is a complete immutable input catalog: 28 outer project aliases, 533 files/sheets/pages, every project `extraction_completeness.json` is `COMPLETE`, `clean_conclusion_allowed=true`, `incomplete_page_count=0`, and every manifest carries the authoritative `input_root`.
- The last successful full-corpus orchestration used one Python process per manifest-derived project with two workers. A single long process and guessed `test/<project>` paths are known failures; output must be interpreted through each project's inner bundle and not a flat outer directory.
- Artifact schemas repeat IDs across projects and include nullable/nested Arrow types, so Phase184 aggregation will add project provenance and canonicalize identity fields rather than raw-concatenate Parquet tables.
- The current full-corpus issue total is intentionally unknown until the new extraction and inner-directory audits complete; the Phase182 `55 -> 45` result remains the comparison baseline only.
- Phase184 fresh and audit completed across all 28 partitions. Independent and main-thread validators prove 533 files/sheets/pages, 533 valid, zero invalid/incomplete, and all required per-project artifacts.
- Pair totals are `38,814 -> 38,825`. Only 23000 (`2296 -> 2303`), 27000 (`680 -> 684`) and PAC (2012 unchanged, one semantic replacement) differ. The comma changes replace six ordinary/semantic rows with six `DYQK` wire-component mappings per affected project plus split-group discard evidence; PAC changes only `PW0147` to terminal-range semantic review. The other 25 projects are stable-semantic exact.
- Current audit is 34 issues: many-to-one `12`, missing-side `18`, cross-page `3`, low-confidence `1`. Exact identity comparison shows two additions and thirteen removals because 23000 Pair IDs after the comma insertion shifted (`PW0493/PW0519 -> PW0500/PW0526`); content identity has zero additions and eleven real removals.
- The eleven removals are nine complete comma/KLP shared-endpoint reviews, 23000 low-confidence `1309 -> 10` recovered by DYQK mapping, and PAC `PW0147 ? -> 431` recovered as terminal-range semantic review. No unrelated issue was hidden.
- The full runner's interactive shell timed out after 120 seconds while its Python parent and two workers continued. The closed stdout pipe froze the status writer at 13/28, but workers completed 28 fresh and 28 audits and exited naturally. Final acceptance comes from direct bundle validation, not the stale status file.
- Residual clustering selected one narrow complete-authority family: PAC has five groups with exactly one authoritative table mapping plus one geometry-owned accessory component mapping on different files/sheets. Three have no component-page outgoing chain; two have raw comma outgoing groups whose normalized targets exactly close the incoming graph.
- Geometry-owned evidence is block/instance/port based rather than line-group based. The current helper rejects it only because it demands `line_group_id/supporting_line_ids`; the producer already supplies body/port text IDs, block name+handle, external text+coord, recognition mode, exact raw split, and explicit `internal_connectivity_inferred=false`, `electrical_union_eligible=false`, `ordinary_pair_eligible=false`.
- The five target primaries are PAC `PTMR0057`, `PTMR0059`, `P0129`, `P0178`, and `P0179`. Their related component mappings are `PAB0100`, `PAB0162`, `PAB0010`, `PAB0014`, and `PAB0012`; outgoing comma closure is provided by `PAB0103/PAB0104` and `PAB0164/PAB0165`.
- Seven structured groups remain explicit negatives: PAC `1ID13` and `GND`, WBH `KD26` three-input and non-closing `PCM0101`, 23000 same-scoped-n `PCM0082`, and the 30000/8000 GND groups. No broad GND, cross-page, name-based or comma-presence exemption is authorized.
- A separate read-only operation-box audit found five CONNECT multi-drop rails and one closed-frame edge behind six missing-side reviews, but implementing a generic topology shadow has a larger blast radius. It is deferred in favor of the existing narrow cross-diagram contract extension.
- Implemented `_authoritative_geometry_owned_component_identity()` in `rules.py` and wired it into the existing component/table cross-diagram and comma-chain gates. The helper requires geometry-owned accessory mapping mode, complete block/body/port/text/coordinate evidence, exact logical/right/raw split values, confidence/pass, and all three connectivity eligibility flags explicitly false.
- Added geometry-owned positive, comma-closure, missing-authority, bad-flag, incomplete-chain and extra-table tests. The focused rules gate is `116 passed`; no Pair extraction code changed.
- Independent read-only review rejected the first draft as too broad. Producer evidence supports exactly four geometry-owned triples: two-port capsule, opposed-port panel, inline two-port, and four-port contact; prefix matching could admit unknown or mismatched producers.
- The first draft also exposed geometry authority through the generic comma-cardinality helper. That path is shared by one-to-many, many-to-one, and duplicate rules, while its historical comma test proves only that some raw label contains a comma. Geometry-owned comma authority must instead prove one shared raw text/coordinate group, split-token membership, and complete normalized token coverage.
- Real PAC target rows prove Pair top-level `left_text_id/right_text_id` and coordinates are exact copies of component-port/external-endpoint evidence. The geometry contract can therefore require those cross-layer identities; the initial fixture accidentally populated candidate IDs while leaving Pair text IDs null.
- The initial positive fixture covered only `accessory_backplate_two_port / geometry_owned_two_port_capsule / strip_two_port_component`. Final coverage must exercise all four exact producer triples and malformed tuple, raw-token, text-ID, and coordinate negatives before replay is authoritative.
- The first tight artifact was still provisional: a legacy `_is_geometry_owned_accessory_component_group()` branch could bypass the strict validator for one-to-many/many-to-one, and the historical generic comma validator could accept geometry rows if legacy line provenance was injected. PAC's 20 valid capsule many-to-one adjacency groups show the old branch cannot simply be deleted; it now delegates to strict geometry identity plus all-pairs raw-group closure, while generic comma authority rejects accessory/geometry modes.
- Final Phase184 authority is `.tmp/phase184_full_533_audit_tight`: 28/28 audit jobs succeeded, every project has JSON/Parquet/Markdown-aligned issue counts, and content identity against `.tmp/phase184_full_533_audit` is `0 additions / 5 removals`. The removals are PAC `PTMR0057/PAB0100`, `PTMR0059/PAB0162`, `P0129/PAB0010`, `P0178/PAB0014`, and `P0179/PAB0012`; no GND, cross-page, missing-side, low-confidence, or KLP review was hidden.
- Final corpus evidence remains `.tmp/phase184_full_533_fresh`: 28 projects, 533 files/sheets, 533 valid, zero invalid/incomplete, Pair total `38,825`. Final issue distribution is many-to-one `7`, missing-side `18`, cross-page `3`, low-confidence `1` (`29` total). Full repository gates pass `1167 passed, 1 skipped`; compileall and diff-check pass.
- The implementation now uses exact producer triples, raw ASCII/fullwidth token closure, same raw text/left/right identity, Pair top-level/evidence cross-checks, and strict all-pairs closure for cross-sheet geometry adjacency. Historical generic comma authority explicitly rejects accessory/geometry modes; all remaining 29 issues stay fail-closed for the next loop.
- Final independent contract review found and closed one remaining dispatch gap: a malformed geometry Pair with forged legacy line IDs could fall back to `_authoritative_strip_component_identity`. Geometry/accessory mode signals now dispatch exclusively to the strict helper, with a combined line-spoof plus top-level-mismatch regression.

## Phase 185 artifact recovery and Phase 186 entry (2026-07-21)

- A concurrent cleanup process removed the entire `.tmp` tree after the Phase185 commit, including the previously accepted fresh/audit artifacts. The cleanup was outside this recognition boundary; `.planning/cleanup_generated_artifacts/` and root `package-lock.json` were left untouched.
- Replayed original inputs using the repository's proven direct-DWG-root discovery: 28 project roots and 533 DWG files. `analyze-project` completed with return code 0 for every root, and `run-audit` completed with return code 0 for every rebuilt findings bundle.
- Retained current artifacts are `.tmp/phase185_full_533_fresh` and `.tmp/phase185_full_533_audit`; only the temporary rebuild logs were removed. The first audit probe used the outer analyze directory and found zero direct `findings` children; no audit output was created by that probe. The corrected probe discovered each inner `pairs.parquet` bundle recursively.
- Fresh artifact validation is authoritative on disk: 28 bundles, 533 files/sheets/valid, zero invalid/incomplete, 28 clean conclusions, 38,825 Pair rows, and zero JSON evidence parse failures. `connect_multidrop_rail` is exactly ten rows: 23000_A `PW0500`; 26000_220kV `PW0117/PW0127/PW0148/PW0149`; 31000_32000_1_2_220kV_A `PW0236/PW0255/PW0256/PW0259/PW0260`.
- Current audit validation is authoritative on disk: 28 projects, 19 issues, and zero JSON/Parquet/Markdown count mismatches. Distribution is many-to-one `7`, missing-side `8`, cross-page `3`, and low-confidence `1`. The pre-cleanup independent audit proved the content identity delta `29 -> 19` with zero additions and ten rail removals; that historical comparison is not recomputed from a deleted baseline.
- The thread limit prevented a second fresh-context audit probe in this recovery pass; the main-thread structured validator and the completed fresh probe are the acceptance evidence. Phase186 remains fail-closed over all 19 rows.

## Phase 186 parallel separator slice (2026-07-21)

- The residual census found exactly two PAC missing-side candidates (`PW0146` and `PW0148`) in a producer geometry family of three same-span, same-layer, pure `LINE` horizontal groups. The two valued rows share left value `211` and text ID `T1008`; the third row (`GW0151`) has no endpoint claim. No other current residual or corpus Pair matched this family.
- Added `_shadow_parallel_grid_separator_ordinary_pairs()` in `page_extractors.py`, called only on the WireDiagramExtractor path. The contract is project/name independent and requires at least three local parallel rows, uniform row spacing within tolerance, two or more repeated side/value/text claims, one fully unvalued discarded row, non-CONNECT non-block `LINE` ownership, exact Pair/evidence selected candidate/raw/text identity, no alternatives/ambiguity/discarded valued claims, and no interior vertical touch. Boundary touches remain allowed; complete pairs, text-only rows, uneven spacing, CONNECT/block/polyline rows, and forged evidence fail closed.
- Added one positive and nine natural-negative unit cases. The focused extractor gate is `84 passed`; the full repository gate is `1190 passed, 1 skipped`.
- PAC bounded replay preserved all `2,012` Pair IDs and persistent columns, shadowed exactly `PW0146/PW0148`, and changed audit `6 -> 4` with zero additions. Full replay preserved all `38,825` Pair IDs and persistent columns, shadowed exactly those two new rows in addition to the existing ten rails, and changed audit `19 -> 17` with zero additions.
- Full fresh validation is `28/28` complete, `533/533` valid, zero invalid/incomplete, and 28 clean conclusions. All project JSON/Parquet/Markdown issue counts align. Final issue distribution is many-to-one `7`, missing-side `6`, cross-page `3`, low-confidence `1`.
- Four unrelated 31000 evidence-only changes are closed-polyline floating-point canonicalization (`PW0236/PW0237/PW0260/PW0261`); all Pair persistent fields and audit issue content remain identical. They are not caused by the separator contract.
...
- A bounded missing-side audit found 10 uniform horizontal CONNECT multi-drop rails (23000/26000/31000), 5 closed same-owner polyline/frame edges (23000/8000-9000), 2 explicit PAC grid separators, and one PAC CT line with no qualifying enclosure. The candidate geometry is not present in current `LineGroup`/`RuleContext` contracts; keep all 18 fail-closed until producer annotation or context plumbing is independently designed.
- A bounded many-to-one audit found GND, PAC `1ID13`, WBH `KD26`, and WBH `PCM0101` remain review under current strict evidence. Main-thread verification corrected one delegated overstatement: WBH `P0243` also lacks `plugin_slot/plugin_title`, so leading-zero normalization cannot authorize it. `23000 PCM0082` has a narrower `"09"` versus integer `9` mismatch, but it remains explicitly fail-closed in this loop pending separate authority.
- The three cross-page and one low-confidence rows were not re-audited in this bounded round because the third agent slot was unavailable; their Phase184 fail-closed disposition remains authoritative.
- Selected Phase185 producer slice: horizontal ordinary half-pairs on pure, non-block-owned `LINE/CONNECT` groups with at least two distinct interior non-block-owned vertical `LINE/CONNECT` segments whose endpoint (not mere crossing) touches the rail. The page extractor has raw lines at the existing shadow stage, so no model or RuleContext widening is required.
- The rail contract also requires one and only one valued/text-backed side, no alternative Pair candidates, no ambiguity gap, strict interior x positions, and distinct drop x coordinates. Complete pairs, one-drop groups, crossings, coincident drops, non-CONNECT lines, block-owned lines, and endpoint-border touches stay eligible.
- Added `_shadow_connect_multidrop_rail_ordinary_pairs()` in `page_extractors.py` and positive/adversarial coverage in `test_page_extractors.py`; focused gate is `67 passed`. Affected raw replay for 23000/26000/31000 is the next authority gate.
- Independent rail review found one tolerance-edge bug: a near-vertical drop used its x midpoint, so a touching endpoint inside the rail-edge exclusion could be shifted inward. The final contract uses the actual rail-touching endpoint x for interior and distinctness checks; tests also cover slanted edge touches, alternatives, ambiguity, block-owned drops and LWPOLYLINE drops.
- Final Phase185 extraction `.tmp/phase185_full_533_fresh` is complete for 28 projects / 533 files and sheets: 533 valid, zero invalid/incomplete, Pair total `38,825`, and zero Pair ID or stable semantic drift against Phase184.
- Full-corpus rail authority is exactly ten Pairs: 23000 `PW0500`; 26000 `PW0117/PW0127/PW0148/PW0149`; 31000 `PW0236/PW0255/PW0256/PW0259/PW0260`. No other project or Pair receives `connect_multidrop_rail` shadow evidence.
- Final audit `.tmp/phase185_full_533_audit` changes `29 -> 19` with content identity `0 additions / 10 removals`; JSON, Parquet and Markdown counts align in all 28 projects. Distribution is many-to-one `7`, missing-side `8`, cross-page `3`, low-confidence `1`.
- The remaining eight missing-side rows are five closed/frame edges not covered by this rail contract, two explicit PAC grid separators, and the unresolved PAC CT line. Together with all seven many-to-one, three cross-page and one low-confidence rows, they remain fail-closed for the next evidence loop.
- Final gates: page extractors `72 passed`; full repository `1178 passed, 1 skipped`; compileall and diff-check pass.

## Phase 188 recovery and evidence loop (2026-07-22)

- Recovered `HEAD=108cd00` on synchronized `master` with a clean tracked worktree; root `package-lock.json` is the only visible untracked file and remains out of scope.
- Authoritative inputs remain `.tmp/phase187_full_533_fresh` and `.tmp/phase187_full_533_audit`: 28 projects, 533 valid DWG, Pair total `38,825`, and 13 issues distributed `7/2/3/1` across many-to-one/missing-side/cross-page/low-confidence.
- Phase188 begins with independent raw-evidence audits of 23000 `PW0526` and PAC `PW0001`. Neither issue is an optimization target by default; selection requires a complete producer-owned contract plus a full-corpus census.
- Independent 23000 audit locates `PW0526/GW0526` on `S0020/F0020`, `20 高压侧操作箱原理图2.dwg`: ordinary `802 -> ?`, left `CW2588/T2223`, right candidate/text absent. Its line is one edge of a single closed LWPOLYLINE, but no mapping, continuation, source block, row band or reciprocal producer evidence exists. Adjacent `PW0524` reuses the same `802/T2223` anchor, so a single-frame or text-anchor exemption would be unsafe. Keep `PW0526` review.
- Independent PAC audit locates `PW0001/GW0001` on `S0005/F0005`, `05 发变组保护CT参考极性图.dwg`: ordinary `? -> 3`, left candidate/text absent, right `CW0006/T0029`, and no related Pair or source block. The right candidate has vertical alignment score `0.0` and a `7.43` y-offset; nearby `PW0002` is a fully unvalued short line. Keep `PW0001` review.
- The 13-row inventory is exact: missing-side 23000 `PW0526` and PAC `PW0001`; low-confidence 31000 `PW0225`; cross-page 24000 `P0076`, 25000 `P0034`, PAC `P0208`; many-to-one 23000 `PCM0082`, 30000 `PTMR0030`, 8000/9000 `PTMR0165`, PAC `PTMR0235/PTMR0236`, and WBH `PCM0042/PCM0101`.
- The selected evidence-audit order is low-confidence self-loop, non-GND structured convergence, then GND/`1ID13` table convergence. Cross-page rows keep their existing fail-closed disposition pending an exact header/scope recheck.
- 31000 `PW0225/GW0225` is a deliberate low-confidence review, not a shadow candidate. Both sides select raw `932`, but the Pair carries alternatives `PCW0272/PCW0273` and `ambiguity_gap=0.1662`; the existing producer shadow contract correctly rejects any alternatives or non-null gap. Equal endpoint text cannot authorize auto-pass.
- 23000 `PCM0082` and WBH `PCM0042/PCM0101` have enough exact component/table evidence to classify their review families, but not enough to prove endpoint equivalence. `PCM0082/PCM0101` combine `backplate_virtual_table + strip_two_port_component`; `PCM0042` combines `terminal_header_table + strip_two_port_component`. Header/plugin/row and component-body/port/line evidence must remain prerequisites, yet their intersection still supports review only.
- 30000 and 8000/9000 GND groups share only normalized text. Their headers, row values, text IDs, coordinates and in one case sheets differ; PAC is the stronger natural negative because one exact logical row emits both `1ID13` and `GND` while the aggregate spans additional rows/sheets. No physical shared endpoint is proven. Exact-GND cardinality suppression still requires explicit human authority and is not selected.
- A delegated cross-page probe proposed splitting otherwise identical backplate rows by `source_block_name`. This is rejected by existing Phase181 corpus evidence: 24000, 25000 and PAC deliberately reuse one explicit header across different source blocks without distinct plugin/device authority. Different block names alone do not prove different physical scopes; all three cross-page reviews remain fail-closed.
- Main-thread PAC S0005 geometry inspection finds only `GW0001` (150-unit horizontal `LINE`, `? -> 3`) and `GW0002` (50-unit horizontal `LINE`, `? -> ?`). There is no repeated parent, uniform row array, structured mapping or reciprocal producer family. `PW0001` remains review.
- A narrower 23000 hypothesis remains to test: `PW0526` is a closed-frame edge and reuses `802/T2223` with adjacent `PW0524`. If `PW0524` is an independently owned electrical line with strictly better/exclusive text identity, a generalized canonical-line-versus-frame contract may exist; this requires exact evidence review and a 38,825-Pair census before implementation.
- The 23000 hypothesis is rejected by raw evidence: `PW0524` selects `702 -> 1-4C1D2` as a discarded `wire_component_mapping`; its `802/T2223` candidate is only rank 3 and an alternative, not an authoritative line. `PW0526` is one closed LWPOLYLINE parent with two nearby CONNECT members (`L3383/L3384` and `L3487/L3488` geometry), so no unique CONNECT authority exists. Text reuse cannot shadow the frame safely.
- Phase188 candidate census conclusion: all 13 active issues have been independently dispositioned and no producer-backed contract currently supports a generalized automatic removal. Preserve all 13 fail-closed issues; do not add a project/name/handle/row-specific rule.
- Bounded current-rules audit replay over Phase187 23000/PAC findings is identity-exact: 23000 remains `2 -> 2`, PAC remains `4 -> 4`, with zero content additions/removals. `PW0526 802 -> ?` and `PW0001 ? -> 3` remain present.
- Full Phase187 artifact validation passes: 28 manifests, 533 files/sheets/valid, zero invalid/incomplete, 28 clean conclusions, 38,825 Pair rows, and 28 audit bundles with JSON/Parquet/Markdown counts aligned. Distribution is `7/2/3/1`.
- Phase186→187 issue identity is `0 additions / 4 removals` (only 8000/9000 `PW0018/PW0019/PW0035/PW0036`). Project-qualified Pair identities are `0/0` delta and all 25 non-evidence persistent columns have zero drift.
- Repository gates after the Phase188 evidence-only loop: `python -m pytest -q` -> `1221 passed, 1 skipped`; `python -m compileall -q src tests` passed; `git diff --check` passed. No source/test behavior changed.

## Phase 189 producer-authority hypotheses (2026-07-22)

- Recovered `HEAD=512186d` synchronized with `origin/master`; tracked worktree is clean and root `package-lock.json` remains the only untracked boundary.
- Phase189 will test two genuinely new authorities rather than repeat Phase188's rejected single-CONNECT/text-reuse ideas: multi-port CONNECT pass-through geometry for the 23000 closed parent, and complete CT-polarity reference semantics for PAC S0005.
- Both hypotheses start fail-closed. A contract must be project/name/handle/coordinate independent, match an exact full-corpus family, and retain every natural negative before production code changes.
- The multi-port enclosure hypothesis is rejected at source-contract review: existing closed-frame evidence does not prove opposite-edge traversal or distinct complete CONNECT paths, and `PW0526` carries no such producer annotation. Do not combine independent unique-CONNECT/repeated-parent/multidrop outcomes into new authority.
- Corrected exact-token census found 57 pages with at least one P1/P2/S1/S2/star token; the preliminary 52-page figure used an unstated narrower filter and is not the authoritative denominator. Only PAC S0005 combines polarity semantics with exact local P1/P2/S1/S2, repeated `*` labels and power-flow semantics; 25000 S0008 has a polarity note but no complete motif or power-flow text, while network-panel P1/P2 pages lack S1/S2 and polarity semantics.
- The candidate Pair-level contract will require both line endpoints to be locally surrounded by a complete P1/P2/S1/S2 plus at least two-star motif, plus independent polarity and power-flow semantic text on the same sheet. A title keyword alone is insufficient.
- Corrected Pair-level census covered all 28 projects / 533 pages / 38,825 Pairs. Of 461 one-sided WireDiagram review rows, only 23000 `PW0526` and PAC `PW0001` lacked a prior producer shadow; the strict raw-line gate excludes `PW0526` because its sole member is LWPOLYLINE. The final complete contract therefore matches exactly PAC `PW0001/GW0001` on `S0005/F0005`, with raw line `L0004`, selected right `CW0006/T0029/3`, endpoint motif IDs `T0023-T0028` and `T0030-T0035`, polarity evidence `T0065/T0083`, and power-flow evidence `T0066`.
- PAC `PW0002` is a same-page natural negative because both sides are empty and the Pair is discard. 25000 S0008 is a polarity-only negative, and 11000 S0006 is a partial P1/P2 network-panel negative. None may be authorized by page title or project identity.
- The implemented `ct_polarity_reference_annotation` contract closes Pair/evidence/TerminalCandidate/TextItem identity, finite insertion and endpoint coordinates, unique page/group/line ownership, one pure non-CONNECT LINE member, Euclidean local motifs with disjoint text identities, and separate same-sheet CT-polarity/power-flow evidence. It explicitly rejects derived numeric candidates, present-side block/channel-detail drift, duplicates and malformed geometry.
- PAC raw replay preserved all 2,012 Pair IDs and every non-evidence persistent column; only `PW0001` evidence changed and it is the sole new CT shadow. PAC audit changed `4 -> 3` with zero additions and exactly one removal: `R-PAIR-MISSING-SIDE`, `S0005/F0005`, `PW0001`, `? -> 3`. The retained PAC issues are `P0208`, `PTMR0235`, and `PTMR0236`.
- Final full original-DWG replay `.tmp/phase189_full_533_fresh` is complete for 28 projects / 533 DWG and sheets: 533 valid, zero invalid/incomplete, 28 clean conclusions, and 38,825 Pair rows. Project-qualified Pair IDs and all non-evidence persistent columns are exact against Phase187; evidence-only differences are the intended PAC `PW0001` annotation plus known 31000 `PW0260/PW0261` canonicalization.
- Final full audit `.tmp/phase189_full_533_audit` has aligned JSON/Parquet/Markdown counts for all 28 projects. Issue identity is `13 -> 12`, with zero additions and exactly one removal: PAC `PW0001` (`R-PAIR-MISSING-SIDE`, `? -> 3`). Distribution is many-to-one `7`, missing-side `1` (23000 `PW0526`), cross-page `3`, low-confidence `1`.
- The remaining 12 issues are explicitly retained as fail-closed next-loop objects. In particular, the 23000 single LWPOLYLINE frame `PW0526`, all three cross-page scope conflicts, all seven many-to-one structures, and the low-confidence row remain visible for separate evidence-authority work.

## Phase 190 residual census and selected contract (2026-07-22)

- Recovered synchronized `HEAD=0f158ef`; the tracked worktree is clean and root `package-lock.json` remains the only untracked boundary. Phase189 fresh/audit artifacts are the authoritative 28-project / 533-DWG / 38,825-Pair baseline with 12 issues.
- The exact residual producer inventory is `P=3` backplate cross-page conflicts, `PW=2` ordinary missing/low-confidence rows, `PTMR=4` terminal-header many-to-one groups and `PCM=3` component/structured many-to-one groups.
- Full-corpus ordinary census rejects both PW rows. `23000 PW0526` is one short LWPOLYLINE frame without a unique producer-owned CONNECT traversal; ten structurally comparable rows include five natural negatives. `31000 PW0225` has three competing candidates and a same-value natural negative `PW0254`; endpoint text equality is not physical ownership.
- Terminal-header census covers 6,607 mappings and 305 pure many-to-one groups. Existing physical-label/reciprocal/scope contracts settle 301 groups; the four residual groups remain. PAC `1ID13` has a plausible reciprocal-cluster extension, but the selected zero-padding defect is narrower and already has a direct same-contract corpus control. GND-only relaxations are rejected.
- Structured component/table census covers 42 shared-endpoint groups; 39 are already settled and three remain. The strongest defect is 23000 `PCM0082 / P0126`, where all authoritative component/table evidence is complete but `row_number=9`, `raw_row_number="09"` and `middle_value="09"` fail string equality. Same-project `PCM0073 / P0128` is the natural control with `11/"11"/"11"` and is already accepted.
- The selected contract changes only row-number representation equivalence. It requires non-boolean integer `row_number` in `1..64` plus native string raw/middle labels exactly equal to its two-digit `:02d` rendering, with no trimming or coercion, then reconstructs the logical endpoint from that same rendering. Numeric evidence objects, whitespace, signs, decimals, mixed text, inconsistent padding and out-of-range values remain ineligible.
- Main-thread corpus census found 4,407 `backplate_virtual_table` rows: all `row_number` values are integers in `1..32`, every raw/middle value is the exact two-digit `:02d` producer rendering, and 1,582 rows use a leading zero. No single-digit or over-width producer row exists. The rules contract therefore mirrors the producer boundary (`int`, non-bool, `1..64`, exact two-digit labels) rather than accepting arbitrary decimal strings.
- WBH `PCM0101/P0243` remains review because its component outgoing graph does not close to all incoming sources. WBH `PCM0042/PTMR0066/PTMR0068` remains review because one component plus two table rows violates the exact cardinality contract. The three cross-page clusters remain review because repeated raw headers and missing device identity lack a safe pair-level scope proof.
- Bounded 23000 fresh replay is complete for 33/33 valid files and 2,303 Pair rows with zero Pair/evidence drift. Its final audit is `2 -> 1`, zero additions, sole removal `PCM0082 + P0126 / 1-4n609`; `PW0526` remains exact.
- Final full fresh replay `.tmp/phase190_full_533_fresh` is complete for 28 projects / 533 files and sheets: all valid, zero invalid/incomplete, 28 clean conclusions and 38,825 Pair rows. Project-qualified IDs and all 25 non-evidence persistent columns are exact against Phase189.
- The only fresh evidence differences are 31000 `PW0236/PW0237/PW0260/PW0261` enclosure/auxiliary floating-point values. They do not change Pair identity, endpoints, status, confidence, graph semantics or audit results.
- Final audit `.tmp/phase190_full_533_audit` has aligned JSON/Parquet/Markdown counts in all 28 projects. Identity is `12 -> 11`, zero additions and exactly one removal: 23000 `R-MANY-TO-ONE`, `PCM0082` related to `P0126`, `1-4CLP1-2 / 1-4n509 -> 1-4n609`.
- Final issue distribution is many-to-one `6`, cross-page `3`, missing-side `1` and low-confidence `1`; all 11 are review. `PW0526`, `PW0225`, four terminal-header groups, two WBH component/table groups and all three cross-page clusters remain fail-closed.
- Final gates after the native-string strictness patch: `tests/unit/test_pairs_and_rules.py` is `120 passed`; repository is `1263 passed, 1 skipped`; compileall and diff-check pass.
- Scoped recognition implementation `e323db9` is published on `origin/master`. Phase190 is complete for this exact producer contract; its 11 residual reviews remain the authoritative Phase191 input rather than accepted suppressions.

## Phase 191 initial baseline (2026-07-22)

- Published Phase190 through acceptance record `775c556`; `HEAD` and `origin/master` match, the tracked worktree is clean, and root `package-lock.json` remains the protected untracked boundary.
- Phase191 starts from `.tmp/phase190_full_533_fresh` and `.tmp/phase190_full_533_audit`: 28 projects, 533 valid files/sheets, 38,825 Pair facts and 11 review issues (many-to-one 6, cross-page 3, missing-side 1, low-confidence 1).
- Initial family split is four terminal-header many-to-one groups, three cross-page conflicts, two ordinary PW rows, and two WBH component/table groups. No family is pre-authorized; independent source/artifact review and natural negatives must precede implementation.
- Three clean-context Sol audits enumerated all 11 identities. Ordinary `23000 PW0526` is a short closed-frame edge without repeated/unique-CONNECT ownership; `31000 PW0225` still has competing candidates. WBH `PCM0042` lacks canonical `KD26/KD-26` identity and complete cardinality, while `PCM0101` lacks a producer-issued junction that closes the component/table fan-in. All four remain review.
- Cross-page `24000 P0076/P0197`, `25000 P0034/P0208` and PAC `P0208/P0280` all fail the existing explicit-scope contract. The first two have no device instance; PAC repeats the same `1n/NCK305` raw identity. Source-block suffixes `-1/-2` identify pages, not independent devices, and existing repeated-header tests require all three to remain review.
- The four pure terminal-header residuals are 30000 `PTMR0030/PTMR0061`, 8000/9000 `PTMR0165/PTMR0342`, and PAC `PTMR0235/PTMR0264/PTMR0307` plus `PTMR0236/PTMR0272/PTMR0281/PTMR0288/PTMR0327`. Each linked Pair has terminal-header row/description/column ownership; their shared values span multiple physical text instances, so the existing same-coordinate helper does not apply.
- Corpus census covers 6,607 terminal-header mappings and 621 many-to-one groups. Existing physical-label or reciprocal closure settles 617; only these four pure terminal-header groups remain. Mixed WBH `KD26` is a decisive natural negative because its cluster includes a component Pair.
- Main-thread source inspection confirms `_high_confidence_pairs()` admits structured table mappings even without pass status. Any Phase191 terminal-header authority helper must therefore independently require `status=pass`, confidence `>=0.95`, the full authoritative table contract, exact pair/mapping endpoint identity, sequence-valid row, `说明` evidence and `endpoint_column_authority=nearest_header_or_named_row`.
- Main-thread identity enumeration reconfirmed exactly 11 audit rows. The four terminal-header issue clusters contain `2/2/3/5` Pair IDs respectively; WBH contains `PCM0101+P0243` and `PCM0042+PTMR0066+PTMR0068`. Cross-page `related_pair_ids` cover their full page conflict clusters, so no future scope rule may validate only the primary/opposite exemplar.
- Corrected strict-predicate census found 621 terminal-header many-to-one groups across 21 projects and 1,246 Pair facts; all 621 independently satisfy the producer contract. Applying the same predicate to the six residual many-to-one issue clusters accepts exactly the four pure terminal-header groups and rejects WBH `PCM0101+P0243` and `PCM0042+PTMR0066+PTMR0068` because they mix component/backplate evidence or lack terminal-column authority.
- The census initially advanced one generalized contract to independent review: every linked Pair would need `pass`/`>=0.95`, authoritative `terminal_header_table`, row-sequence validity, `说明` ownership, exact logical/side endpoint binding and `nearest_header_or_named_row` column authority. This was a candidate only; no behavior was accepted before the later scope review.
- Existing test `test_build_issues_keeps_terminal_only_shared_endpoint_on_generic_many_to_one` is the decisive incomplete-evidence natural negative: it has pure terminal-header Pair kinds but lacks the complete producer identity/description/column contract and must remain a generic review after Phase191.
- Implemented `_is_authoritative_terminal_header_shared_endpoint_group()` as an all-linked-Pair contract. It reuses the single-Pair authoritative table validator, then requires native producer row/coordinate data, exact description and endpoint-column roles, shared-side binding, `pass` and confidence `>=0.95`; mixed component/backplate or malformed evidence cannot qualify.
- Focused Phase191 gate passes `20 passed, 118 deselected`. The positive uses two files/sheets, two physical endpoint coordinates and generic `SHARED-BUS`; 17 adversarial mutations plus the pre-existing incomplete-evidence regression all preserve `R-MANY-TO-ONE`.
- Independent Sol review rejected that implementation before replay. The positive itself proved the fail-open: two unrelated sheets/headers/text IDs/coordinates were treated as one endpoint solely because both strings were `SHARED-BUS`. Terminal row/column ownership is not a producer-issued cross-scope device or electrical identity.
- The review also found `float(confidence) < 0.95` would accept `NaN`/infinity and raise on nonnumeric strings. Rather than harden a semantically invalid contract, the complete Phase191 helper, helper fixture and all new behavior tests were removed. `rules.py` and `test_pairs_and_rules.py` now have zero content diff from `HEAD`; the existing incomplete terminal-only test still passes.
- Phase191 is therefore an evidence-backed no-change round. All 11 Phase190 issues remain authoritative fail-closed inputs; acceptance will use unchanged-artifact identity/parity validation plus repository gates, without redundant raw-DWG extraction.
- Bounded rules audit `.tmp/phase191_fail_closed_audit` exactly matches Phase190 for 30000 `1/1`, 8000/9000 `1/1`, PAC `3/3` and WBH `2/2`. Each project has JSON identity delta `0/0`, full JSON object equality, equal Parquet counts and byte-equal Markdown output.
- Retained full-artifact validation passes: 28 projects, 533 files/sheets/valid, zero invalid/incomplete, 28 `COMPLETE`, 38,825 unique project-qualified Pair IDs and 11 issues. Distribution remains many-to-one 6, cross-page 3, missing-side 1 and low-confidence 1; all severity `review`, with 28-project JSON/Parquet/Markdown count parity.
- Final no-change repository gates pass `1263 passed, 1 skipped`; compileall and diff-check pass. Recognition source/tests have no content diff, so raw-DWG replay would only regenerate the accepted Phase190 corpus without testing a changed behavior.
- Cleanup removed only `.tmp/phase191_fail_closed_audit`; retained Phase190 full fresh/audit artifacts are the Phase192 baseline. Phase191 changes only planning evidence and does not suppress any issue.

## Phase 192 producer-identity baseline (2026-07-22)

- Published Phase191 as `f2aa339`; `HEAD` and `origin/master` match and root `package-lock.json` is still the only untracked boundary.
- Phase192 inherits the unchanged 11 issues and prohibits repeating the rejected rule-layer same-text contract. New work must add or discover producer-issued physical/electrical scope.
- Initial independent probes target PAC `PTMR0235` reciprocal topology, the three multi-physical GND groups, and raw backplate device/plugin identity for `24000 P0076`, `25000 P0034` and PAC `P0208`. None is preselected.
- Main-thread PAC census found the only strict new reciprocal candidate: `PTMR0235/PTMR0264/PTMR0307 -> 1ID13` on S0030. Physical endpoint clusters are `T5316@(158.5,248.5)` carrying `1ID5` and `T5414@(128.5,168.5)` carrying `1ID21` plus `1U2D13`; reciprocal logical `1ID-13` row Pair `PTMR0249/PTMR0250` targets `1ID21` and `1ID5` on left/right columns.
- The strict quotient contract requires at least three authoritative terminal-table Pairs, at least two physical endpoint text/coordinate clusters, exactly one reciprocal target per cluster, reciprocal targets contained in source logical endpoints, and a same-scope reciprocal row with both endpoint columns. Full corpus produced exactly one candidate; `1ID21` was already suppressed by the existing same-scope guard and no GND/WBH/unrelated group qualified.
- GND raw census covers 16 terminal-header `-> GND` mappings across 10 projects and 12 sheets. Every endpoint is an independent `TEXT` on layer `0`, has no `source_block_name`, has a unique text/coordinate identity, and has no logical endpoint pointing back to `GND`; nearby lines are ordinary table grid geometry. No generalized global-ground producer identity is present.
- Backplate source review at `table_extractor.py:_build_backplate_virtual_mappings` (1474+) and `_resolve_backplate_device_instance` (1657-1685) found weak bare `1n` labels in the audit area for 24000/25000 plus title-region `1n REAR WIRING 1/2` texts outside the selected page title. PAC pages explicitly carry the same `1n REAR WIRING 1/2`, while target mappings have no plugin slot/title. `source_block_name` suffixes `-1/-2` are page halves, so no independent device scope can safely suppress the three cross-page conflicts.
- The strict PAC reciprocal quotient is now the only selected Phase192 contract. It is name-independent: physical endpoint text/coordinate clusters are partitioned, each cluster has exactly one reciprocal target from a same-scope two-sided logical row, and all targets are contained in the source set. Full-corpus census yields one new group and no natural-negative collision.
- Resume-time artifact verification reconfirmed the exact producer shape in `.tmp/phase190_full_533_fresh/PAC-885G-H`: `PTMR0235` owns physical cluster `T5316@(158.5,248.5)`, `PTMR0264/PTMR0307` share `T5414@(128.5,168.5)`, and reciprocal `PTMR0249/PTMR0250` share the authoritative `1ID-13` header/row scope while occupying its right/left endpoint columns. Every row carries `nearest_header_or_named_row`, row-sequence, `说明`, text ID and coordinate evidence; this is the concrete producer boundary for the implementation.
- Implemented the reciprocal physical-cluster quotient behind the existing same-row reciprocal guard. It requires exact single sheet/file scope, at least three unique authoritative linked facts, at least two endpoint `text_id+coord` clusters, exactly two same-header/same-physical-middle-row reciprocal facts on opposite columns, unique targets contained in the linked source set, and a bijection from physical clusters to targets.
- Independent diff review identified and the main thread reproduced several initial gaps. The final contract requires nested endpoint/middle text and coordinates to match top-level Pair fields, rejects blank IDs, bounds native finite confidence to `[0.95,1.0]`, avoids integer-to-float coercion, and uses a new terminal reciprocal normalizer that removes only optional hyphens. Semantic `+`, `/`, `_` and `.` characters are not collapsed.
- Pair-ID checks use only generic nonblank uniqueness and never inspect a fixed value; this implements the handoff's duplicate-fact guard without project or Pair-name coupling. A direct reconstruction of the five real PAC Pair objects from Phase190 Parquet returns `True` under the tightened helper.
- Bounded raw PAC replay `.tmp/phase192_pac_fresh` is complete for 31/31 valid DWG with `analysis_status=COMPLETE`. Its 2,012 Pair IDs and every full Parquet row are identical to Phase190. Audit `.tmp/phase192_pac_audit` is `3 -> 2`, additions `0`, with the sole removal `PTMR0235 / 1ID-5 -> 1ID13`; both GND groups remain exact.
- Final full replay `.tmp/phase192_full_533_fresh` is complete for 28 projects / 533 files and sheets: all 533 valid, zero invalid/incomplete, all `COMPLETE`, and 38,825 Pair rows. Project-qualified Pair IDs and every non-evidence semantic column are unchanged from Phase190; only 31000 `PW0260/PW0261` enclosure floating-point evidence varies.
- Final full audit `.tmp/phase192_full_533_audit` has complete JSON/Parquet/Markdown/runtime artifacts for all 28 projects. Identity is `11 -> 10`, additions `0`, and exactly one removal: PAC `R-MANY-TO-ONE`, `PTMR0235`, `1ID-5 -> 1ID13`. Retained issue objects are byte-semantic equal excluding generated issue IDs. Distribution is many-to-one `5`, cross-page `3`, missing-side `1`, low-confidence `1`, all severity `review`.
- Two final Sol review findings were closed before the authoritative audit rerun: top-level Pair coordinates now reject bool aliases before equality checks, and terminal header prefix/header IDs must be native nonblank strings. Separator negatives explicitly cover `+`, `_` and `.`. Final focused rules coverage is 166 passing tests; repository coverage is `1309 passed, 1 skipped` with compileall and diff-check clean.
- Cleanup retained only Phase190 baseline full fresh/audit and Phase192 final full fresh/audit. Bounded PAC artifacts were removed after acceptance. The remaining 10 reviews stay fail-closed: many-to-one `5` (including GND/WBH/other terminal structures), cross-page `3`, missing-side `1`, and low-confidence `1`.

## Phase 193 baseline (2026-07-22)

- Published Phase192 as `f1c6a09`; `HEAD == origin/master` and the tracked worktree was clean before Phase193 planning, with root `package-lock.json` still the sole protected untracked file.
- Phase193 starts from `.tmp/phase192_full_533_fresh` and `.tmp/phase192_full_533_audit`: 28 projects, 533 valid files/sheets, 38,825 Pair facts and 10 review issues (many-to-one 5, cross-page 3, missing-side 1, low-confidence 1).
- Completed Phase192 negative evidence remains authoritative: GND lacks block/layer/network/reciprocal ownership, and cross-page backplate page suffix/title repetition does not establish independent device scope. Phase193 will not retry those rule-layer hypotheses.

## Phase 193 residual producer audits and closed-polyline selection (2026-07-22)

- Three clean-context `gpt-5.6-sol` audits are no-change. WBH `PCM0101/P0243` has apparent reverse edges but incomplete fan-in closure (`5n/NTZ302A-1` versus `5n601`), while `PCM0042/PTMR0066/PTMR0068` has no exact reverse and violates the component-plus-two-table cardinality contract. The 30000 `PTMR0030/PTMR0061` and 8000/9000 `PTMR0165/PTMR0342` groups are separate local GND inserts/cells with no jumper or reciprocal identity. `31000 PW0225/GW0225` is a three-port net and needs future multi-endpoint producer semantics.
- Ordinary `23000_A PW0526/GW0526` is not an electrically authoritative line: `L3404/19F5D:1` is the complete top edge of a closed four-edge dashed rectangle containing only `TC1`; candidate `802` belongs to a separate terminal INSERT/CONNECT outside the rectangle. This is a producer lineage defect, not a confidence threshold issue.
- Full-corpus tolerance census over `.tmp/phase192_full_533_fresh` used the enclosure tolerance `0.25`: 461 WireDiagram ordinary one-sided review rows reduced to 27 direct top-level polyline candidates, 26 open-parent natural negatives and exactly one complete match (`23000_A/S0020/F0020/PW0526`, parent `19F5D`, member `L3404`). The match has closed flag `1`, four vertices, axis-aligned bbox, explicit linetype `虚线`, complete bbox-edge membership, selected candidate outside the bbox, exactly one nonnumeric interior label `TC1`, zero numeric labels, `alternative_pair_candidate_ids=[]` and `ambiguity_gap=null`.
- The bounded candidate implementation was a WireDiagram-only producer shadow. It preserved Pair identity/evidence, suppressed only `ordinary_pair_eligible`, set a dedicated `ordinary_pair_shadow_only` reason/evidence, and never inferred electrical union or deleted the Pair. Missing polyline lineage, noncontinuous linetype, non-rectangular/closed geometry, interior numeric text, candidate ambiguity or malformed native fields failed closed.
- Phase192 project bundles persist Pair/group/line/candidate/text tables but do not contain a `findings/polylines.parquet` file even though `CadExtractionResult` already carries `polylines` in memory. The implementation therefore routes that existing in-memory collection into WireDiagram extraction; bounded replay, not a guessed historical table, is the authority for the new linetype field.
- The rejected draft helper never deleted or retyped the Pair. It set `ordinary_pair_eligible=False`, `ordinary_pair_shadow_only=True` and `ordinary_pair_shadow_reason=closed_graphic_frame`, with parent polyline/member/linetype/bbox/interior-label evidence. Focused coverage rejected open/control/malformed linetypes, multiple members, nonrectangular edges, interior numeric text, missing label, inside candidate, ambiguity, duplicate parent and source-type mismatch.
- Bounded 23000_A raw replay validates the real producer boundary: 33/33 valid and complete, 2,303 Pair rows with exact IDs/all non-evidence columns, evidence delta only `PW0526`, and audit `1 -> 0` with no additions. The shadow payload is `PL0934/19F5D/L3404`, linetype `虚线`, bbox `(81.25,190.00698)-(181.87284,200.00698)`, interior label `T2067`.
- Independent Sol review rejects publication of that bounded behavior. The exact predicate still conflates graphic frames with real dashed closed electrical equipment/shield boundaries; an interior `TC1` or `K1` identifier plus one outside terminal is a plausible genuine electrical negative. Current-corpus uniqueness is not independent semantic authority.
- A second high-severity gap is producer-level: `cad_extract.py` reads LWPOLYLINE `xy` only and drops bulge, so four chord endpoints can look like a straight axis-aligned rectangle even when one or more segments are arcs. Medium/low draft gaps also cover bool/nonnumeric/nonfinite confidence/score, raw versus effective BYLAYER linetype, inverted text bbox and blank/nonunique polyline IDs.
- The candidate is withdrawn before full replay. `PW0526` remains a fail-closed missing-side issue; a future attempt needs independently extracted graphic-only/electrical ownership semantics and straight-edge/bulge evidence, not more thresholds around the same rectangle/text pattern.

## Phase 194 multi-endpoint baseline (2026-07-22)

- Phase193 is published as no-change `e6185ab`; `HEAD == origin/master`, the tracked worktree was clean at transition, and root `package-lock.json` remains the only protected untracked file.
- Phase194 inherits all 10 Phase192 issues and selects no rule-layer relaxation. Its first producer question is `31000 PW0225/GW0225`, where the line passes through three terminal symbols carrying `932` but endpoint-only candidate search emits only the left/right two-port Pair.
- A valid solution must identify symbol-owned physical ports intersecting one network and preserve the network as multi-endpoint semantics. It must not reduce the net to pairwise text equality, treat ordinary crossings as junctions, or hide `PW0526` and other one-sided/closed-frame structures.
- Raw target evidence is complete: `GW0225` member `L1583` is LINE handle `3317C`; left/middle/right terminal INSERT/TEXT handles are `33141/33142`, `3314E/3314F`, `3315E/3315F`, all `SYMB2_M_PWF165`/`932`. Their block-defined transformed ports are `(292.5,147.5)`, `(347.5,147.5)`, `(403.17282,147.5)`, within `0.062` of the network line. XRECORD independently labels each `装置端子` and binds CONNECT handles `33143/33150/33160`.
- The missing middle endpoint is a producer omission: `T1010` is a numeric text but `terminal_candidates.parquet` contains no candidate for it. `candidates.py:_endpoints_for_group` exposes only horizontal left/right endpoints, so the interior port falls outside both endpoint search boxes.
- A preliminary corpus census yielded 30 >=3 transformed-port groups: twenty-eight already-shadowed grid groups, one already-shadowed closed tall enclosure, and only `PW0225` as an eligible ordinary open-line group. However, its 368-DXF scope count is not authoritative. The 28 Phase192 manifests reference 442 distinct existing converted DXFs for 533 source/page rows, so the group classification must be rerun across those 442 paths before selection.
- Raw CONNECT lines `32FBD` and `32FF9` cross the target line but have no terminal INSERT, transformed block port, terminal text or XRECORD `装置端子/CONNECTLINEHANDLE` binding. They are same-sheet geometry negatives that prohibit treating every line intersection as a port.
- Manifest-authoritative 442-DXF metadata census reads every file. It finds 7,131 `装置端子` INSERTs, 6,971 reciprocal INSERT/TEXT identities, 3,484 unique XRECORD/CONNECT resolutions and 1,509 fully strict one-CONNECT/one-child-port bindings; all 1,509 resolved CONNECT entities are on layer `CONNECT`.
- Projected onto current line groups, 1,491 strict bindings occur in files with groups: 496 uniquely match one group at tolerance `0.25`, 995 match none, and zero match multiple groups. Exactly one group has at least three distinct owned ports: `31000_32000_1_2_220kV_A/F0013/S0013/GW0225`, containing `33141/33142/33143`, `3314E/3314F/33150`, and `3315E/3315F/33160`. Its current Pair is `PW0225`, ordinary/review, `932 -> 932`, with no prior shadow. This is the selected name-independent producer contract.
- Re-running the census through the final production parser, after same-appid reciprocity/LINENO cardinality/type/owner hardening, produces the same 1,509 bindings and exact group separation: 496 unique group hits, 995 no-group hits, 18 bindings on files without groups, zero group ambiguity, and only target `GW0225` with three or more ports. The predicate therefore remains corpus-separating without any fixed project, block, value or handle condition.
- Exact target path is `.tmp/phase192_full_533_fresh/cache/converted_dxf/F0013_3c0e5117.dxf`. Its OBJECTS section starts near raw line 41474; global dictionary handle `E83` maps `LD_SYMB2LOGGEN_39/40/41` to XRECORDs `349F8/349F9/349FA`. Those records bind terminal INSERTs `33141/3314E/3315E` to `CONNECTLINEHANDLE` values `33143/33150/33160` and mark each `装置端子`; the INSERT entities also carry `LD_SYMB2_SPECIAL/装置端子` XDATA.
- `ezdxf_reader.py` retains the complete parsed document in `CadDocument.native_document`, and `cad_extract.py` obtains it, but repository source has no current `CONNECTLINEHANDLE`, XRECORD, OBJECTS dictionary, extension-dictionary, or `get_xdata()` consumer. Phase194 must add producer metadata at the CAD extraction boundary or close no-change; downstream rule suppression is not eligible.
- Structured ezdxf inspection shows each target XRECORD stores alternating key/value strings using group code `1`; even `CONNECTLINEHANDLE` is not a native `1005` handle tag. `PART2_1` is a vendor `||`-delimited compound string. An authoritative parser therefore cannot trust a substring or positional split alone: it must require a well-formed alternating key/value record, resolve the referenced entities by exact handle/type, and cross-check INSERT XDATA.
- Each terminal INSERT has `LD_SYMB2_SPECIAL = 装置端子` and two `LD_SYMB2_TERM_TEXT_*` XDATA records pointing by native `1005` handle to the same TEXT. The TEXT reciprocally points to the INSERT. The `CONNECTLINEHANDLE` resolves to a top-level `LINE` on layer `CONNECT`. Crossing negatives `32FBD/32FF9` are also CONNECT lines but have no XDATA and no terminal INSERT/text ownership, so layer or geometric intersection alone is insufficient.
- Target geometry closes a unique continuous ownership chain. Each XRECORD-bound CONNECT line terminates at its terminal INSERT origin (`33143 -> 33141`, `33150 -> 3314E`, `33160 -> 3315E`); each INSERT's first virtual LINE starts at that same origin and ends at the network-facing physical port. The three port endpoints lie within about `0.062` of LINE `3317C`. Crossing negatives intersect the network geometrically but have no terminal origin or child-port chain.

### Phase 194 bounded acceptance

- Bounded raw replay `.tmp/phase194_31000_fresh1` is `COMPLETE`: 25/25 manifest pages, 21 extractor-complete pages, four stable `NOT_APPLICABLE` pages and zero incomplete pages. Baseline/current Pair rows are `1,406/1,406`; Pair order and every non-evidence column are exactly equal.
- Only `PW0225` receives the authoritative multi-endpoint shadow. It remains `review`, confidence `0.7777`, values `932 -> 932` and alternatives `PCW0272/PCW0273`; membership contains exactly INSERT/TEXT/CONNECT handles `33141/33142/33143`, `3314E/3314F/33150`, and `3315E/3315F/33160`. The payload explicitly records complete membership, no canonical endpoint, no pairwise relations, no inferred internal connectivity and no electrical union eligibility.
- Fresh ODA conversion changes only unrelated enclosure evidence floats on `PW0236/PW0237` (`2.500 -> 2.507` and `195.0 -> 195.000001`); Pair semantics and audit identity do not change. Audit identity is `1 -> 0`, additions zero, with sole removal `R-PAIR-LOW-CONFIDENCE/PW0225/GW0225`.
- Pre-full repository gate passes `1360 passed, 1 skipped`; compileall and diff-check pass. Concurrent desktop/preview worktree changes and `.planning/render_preview_drag_assessment/` are unrelated protected boundaries alongside root `package-lock.json` and must remain unstaged/unmodified.

### Phase 194 full acceptance

- Full fresh replay `.tmp/phase194_full_533_fresh` completes 28/28 projects and 533/533 files/sheets: 533 valid, zero invalid, zero incomplete, all 28 projects `COMPLETE`, and 38,825 Pair rows. Project-qualified Pair identity additions/removals are `0/0`; every non-evidence Pair column is exact against Phase192.
- Exactly three evidence rows differ, all in 31000: target `PW0225` receives the complete producer shadow, while `PW0236/PW0237` retain the same two ODA enclosure float jitters seen in bounded replay. Across all 38,825 Pairs, the multi-endpoint shadow occurs exactly once: `31000/S0013/F0013/PW0225/GW0225`, with three ports.
- Full audit `.tmp/phase194_full_533_audit` contains all four artifacts for each of 28 projects and has JSON/Parquet/Markdown count parity. Identity is `10 -> 9`, additions zero, sole removal `R-PAIR-LOW-CONFIDENCE/PW0225/GW0225/932 -> 932`; every retained issue object is exactly equal to Phase192. Final distribution is many-to-one 5, cross-page 3 and missing-side 1.
- The remaining nine reviews stay fail-closed. No global GND, closed-frame, text-equality, page-suffix, fixed block/value/handle, or pairwise-union relaxation is introduced.
- Final independent read-only review confirmed sheet/file/group scope and no implicit union. Its only actionable identity gap was that selected `TextItem.text` was not directly rechecked against the producer binding. The matcher now requires that equality; a stale-text-handle adversarial test fails closed, focused coverage is `48 passed`, and a new bounded 31000 raw replay still yields exactly one three-port shadow and zero issues.
- The review's coincident-LineGroup concern does not justify a new raw-handle rule: the authoritative chain is CONNECT-to-INSERT-origin, unique outward child line to physical port, then unique LineGroup intersection. A second overlapping group already makes membership ambiguous and rejects it; a sole line geometrically passing through all three physical ports is the network in the extracted CAD topology. No name, value or handle-family shortcut is added.
- Cleanup retains only `.tmp/phase194_full_533_fresh` and `.tmp/phase194_full_533_audit`. Final repository gates pass `1361 passed, 1 skipped`; compileall and diff-check pass.

## Phase 195 formal-design corpus and unattended-loop contract (2026-08-16)

- The user explicitly requires a persistent unattended optimization loop over `test/正式典设`, continuing until the user personally pauses/stops it. One accepted slice is never the overall completion condition.
- Initial read-only corpus inventory: 8 top-level projects, 66 recursive directories, 1,419 files, 1,212 DWG, 96 XML, 4 XLSX, 1 RAR; all 1,212 DWG pass the quick non-empty `AC10xx` header check. One leaf directory is empty. The corpus contains no reusable manifest/cache/JSON/Parquet, so the first baseline must be generated by the current engine.
- Recommended first bounded leaf is under GJ-73 (15-30 DWG) or another directory containing one `.prj`; do not pass a multi-project parent directory as a bounded input. CLI evidence: `src/dwg_audit/cli.py:332-347` (`analyze-project`), `src/dwg_audit/cli.py:362-374` (`run-audit`), artifact layout `src/dwg_audit/report/artifacts.py:530-558`.
- High-probability number-group loss boundary: `cad_extract.py:327-355` creates one `TextItem` per TEXT/MTEXT/ATTRIB; `domain/models.py:86-107` stores one normalized scalar; `candidates.py:1317-1329` extracts one numeric value; `audit/pairs.py:22-34,63-72,141-153` aggregates scalar candidates and does not expand multi-number text. This is a hypothesis until raw formal-design replay and visual/raw evidence confirm it.
- Required visual evidence fields for every queued miss/merge/split: source page/region, expected groups, recognized groups, false/ambiguous groups, rendered crop reference, raw entity/block/text handles, candidate/pair IDs, and producer-vs-downstream disposition. Visual observations are leads; raw CAD corroboration is required before implementation.
- First bounded baseline is `.tmp/phase195_formal_gj73_single_fresh` plus `_audit`: GJ-73 `110kV线路保护柜（单跳圈）`, 15/15 valid DWG, three stable skipped pages, 12 COMPLETE analyzed pages, 1,063 Pair facts and three audit reviews. The annotated visual bundle is `.tmp/phase195_formal_gj73_single_visual`.
- Page-by-page visual overlay found the first producer candidate on S0013 `13 元件接线图2.dwg`: components `1CLP4` and `1KLP11` visually match neighboring two-port components but emit only ordinary missing-side Pairs `PC0060 ? -> 922` and `PC0070 ? -> 401`. High-resolution crops are `.../crops/PC0060.png` and `PC0070.png`.
- In the source crop, the missing top-side semantic texts are `1KD5 &` and `1KLP10-1 &`; the same page successfully extracts neighboring component mappings whose external endpoint text is comma-separated and ends in `&` (for example `1KLP1-1,1KLP3-1 &`). Hypothesis: the component endpoint parser accepts the list form but drops the singleton-plus-terminator form. This is not authorized for implementation until raw DXF handles/geometry and adversarial negatives are confirmed.
- Three bounded probes confirm the candidate. Raw S0013 DXF/F0013 shows `1CLP4` block `FJL-25-2A_Mirror` (BB1B), body T1908, pins T1906/T1905 and vertical wire L2739 [245,240] -> [245,225]; `1KLP11` uses BC9D, body T2008, pins T2006/T2005 and L2959 [325,120] -> [325,105]. Both are geometry-equivalent to successful GC0066/GC0071/GC0076 two-port components. Their top texts are T2021=`1KD5 &` and T2049=`1KLP10-1 &`; both are rejected `not_numeric`, while the bottom 1n922/1n401 candidates are accepted.
- Code audit confirms the sole failure: `src/dwg_audit/audit/component_diagrams.py:924-934` `_strip_external_endpoint_values` calls `_clean_external_endpoint` at `:963`, which strips leading `&` only; trailing `&` survives and fails `_is_valid_external_endpoint` `:971-977`. Existing `_clean_small_port_external_endpoint` `:967` already strips both boundaries. The parser must keep raw evidence, trim decorative boundary `&`, validate each comma fragment, and preserve interior/malformed fragments as negatives.
- Negative census: 113 baseline texts end with `&`; 31 participate in existing pass mappings (14 component + 17 table), and many no-`&` mappings also pass. Safe contract is therefore structural: strip boundary `&` only after the text is selected by component/table geometry/row scope; never turn an isolated `&` or out-of-scope text into a mapping. Required gate includes singleton+`&`, comma-list+`&`, no-`&` positives, ordinary PC0060/PC0070 missing-side negatives, malformed/interior `&`, and isolated `&`.
- Implemented the first Phase195 slice by changing only `_clean_external_endpoint` from leading-only to boundary-only `&` trimming. Pair evidence still stores the unmodified raw endpoint, while interior/isolated `&` remains invalid. Focused component coverage is 30/30 and the full unit gate is `1333 passed, 1 skipped`; compileall and diff-check pass.
- Fresh bounded replay `.tmp/phase195_formal_gj73_single_fresh2` is complete and contains 1,091 Pairs. Stable identity diff is 30 additions and two status replacements: 28 new structure-backed component mappings (14 on S0012 KK slots, 14 on S0013 strip components), plus the two original ordinary missing-side rows changing from `review` to `discard` because their line groups are now consumed by complete component mappings. There are no removed recognized mappings.
- The new mappings include the two visual targets (`1CLP4-1 -> 1KD5`, `1CLP4-2 -> 1n922`, `1KLP11-1 -> 1KLP10-1`, `1KLP11-2 -> 1n401`) and recover the previously lost final member of every comma-list-plus-trailing-`&` strip endpoint. S0012 recovers 14 KK endpoints whose source text has the same boundary decoration.
- Bounded audit `.tmp/phase195_formal_gj73_single_audit2` is empty versus three baseline reviews. The two expected missing-side identities are removed. The third removal is the prior `R-MANY-TO-ONE` cross-page review on `1KLP1-1`; it is a secondary consequence of recovering reciprocal component-chain mappings and requires explicit raw/visual audit before acceptance rather than count-only approval.
- Independent identity audit confirms both manifests are 15/15 valid with byte-identical pages/texts/lines/line_groups/terminal_candidates and no coordinate/ODA jitter. Using multiset key `(sheet_id,file_id,line_group_id,left_value,right_value,pair_kind,pair_key)`, baseline is an exact subset of current: 28 additions, zero deletions, and eight true non-evidence changes (six S0012 internal-pin ordinary rows covered/discarded, plus PC0060/PC0070 review->discard). Pair-ID shifts are bookkeeping only.
- The many-to-one removal is reproducible by direct `build_issues` replay, not an artifact join loss. `rules.py:851-855` now sees outgoing `{1QD2,1KLP2-1}` equal to incoming `{1QD2,1KLP2-1}` from the same raw comma group (`1QD2,1KLP2-1 &`), satisfying the existing component/table cross-diagram contract; the corresponding contract is covered by `tests/unit/test_pairs_and_rules.py:584-634`. PTMR0092 remains an unchanged pass terminal-header mapping.
- Fresh2 visual crops were generated under `F:\workspace\XJToolkit\.tmp\phase195_formal_gj73_single_visual2\crops`. Reviewed `PCM0027.png` and `PCM0034.png`: green mappings land on the `1CLP4`/`1KLP11` ports and their source texts `1KD5 &`/`1KLP10-1 &`, with the matching lower endpoints. Reviewed `PCM0002.png` and `PCM0045.png`: comma-list-plus-`&` source text is geometrically paired to the recovered reciprocal endpoint. Reviewed `PCK0001.png`: the KK3P port-to-`7UD10 &` mapping lands on the source endpoint and its supporting lines. No selected crop shows an out-of-region or textless false mapping.
- A read-only raw-mapping probe timed out without a usable message and was interrupted after the ten-minute guard; identity and direct visual/raw artifact audits above remain the accepted evidence. No source changes were made by probes.
- Full formal replay roots are `.tmp/phase195_formal_full_fresh/p000..p048` with audits `.tmp/phase195_formal_full_audit/p000..p048`. All 49/49 leaf projects and 1,212/1,212 DWG are valid and complete; aggregate Pair facts are 88,265 and no extraction-completeness artifact is incomplete.
- The full-corpus audit is report-parity complete for every leaf. It contains 334 fail-closed review issues: `R-PAIR-LOW-CONFIDENCE` 261, `R-PAIR-MISSING-SIDE` 50, `R-MANY-TO-ONE` 17, `R-CROSS-PAGE-CONFLICT` 5, and `R-DUPLICATE-PAIR` 1. No issue was auto-suppressed by count-only logic. The next evidence cluster is the smallest structural `R-DUPLICATE-PAIR` case at p003/S0011 (`PCS0001`, `AK-1 -> JD1`); it requires positive and adversarial duplicate-line fixtures before any implementation decision.
- Cycle 195-02 evidence proves this duplicate is legitimate cross-role corroboration: `PAB0001/S0004` is a primary schematic `inline_two_port_component`, `PCS0001/S0011` is a supplemental `small_port_box_component`, both independently map `AK` port `1` to `JD1`, and both have raw DXF block/text/line ownership. The full corpus supplies 66 exact schematic+KK natural positives and no prior schematic+small-port guard; small-port-only and same-sheet/different-port cases remain negatives.
- Implemented a strict comparison-only `_is_authoritative_schematic_small_port_component_duplicate` in `src/dwg_audit/audit/rules.py`. It requires exactly two distinct sheets, pass/confidence>=0.95, exact `(component_body, component_port, external_endpoint)` identity, and complete small-port body/port/block/instance-bbox/endpoint/line/support evidence. It does not infer connectivity or suppress same-sheet duplicates. Added one positive plus five malformed/missing-evidence negatives in `tests/unit/test_pairs_and_rules.py`.
- Cycle 195-02 bounded replay `.tmp/phase195_formal_dup_p003_fresh` is Pair-identical to p003 baseline (523 rows, zero key additions/removals and zero non-ID drift). Audit `.tmp/phase195_formal_dup_p003_audit` removes exactly `R-DUPLICATE-PAIR/PCS0001/PAB0001`; the four low-confidence identities are unchanged. Fresh visual crops `.tmp/phase195_formal_dup_p003_visual/crops/PCS0001.png` and `PAB0001.png` show both green mappings landing on their source `JD1` and `AK` port geometry.

### Cycle 195-03 evidence and bounded acceptance

- Three read-only probes converged on the same physical provenance. Raw DXF `F0018_e5c53a67.dxf` INSERT `27381` is `KK2P @ (207.5,250)` with block `B1022`; visible `KZKK` and hidden `2-1n` labels are layered on that one instance. `GC0042`/`L4061,L4063` is the block top edge, not an external fan-in wire. PCK0017/PCS0001 share port `T2850`, endpoint `T3048`/`& JD8`; PCK0018/PCS0003 share `T2851`, `T3050`/`& JD3`. A full 49-project census found 990 KK and 479 small-port component rows but only these two cross-submode raw-anchor overlaps.
- Existing rule helpers intentionally reject this topology because the prior schematic+KK/small contracts require distinct sheets, while the component-table contract requires one table row. A safe comparison-only contract therefore requires exactly two same-sheet/file `component_mapping/pass` rows, one KK and one small-port, different body/logical labels, exact block ID/name/coord, port and endpoint text IDs/coords, raw endpoint, line-group/support IDs, and a valid small-port bbox containing both anchors. No block/body/name shortcut, connectivity inference, or Pair deletion is used; any missing/mismatched field remains `R-MANY-TO-ONE`.
- Main-thread implementation adds `_is_authoritative_same_sheet_kk_small_port_alias` at `src/dwg_audit/audit/rules.py:1455` and wires it into `_is_authoritative_structured_cardinality_group` at `:1320-1326`. Focused tests include the two positive JD8/JD3 fixtures plus eight malformed-provenance negatives and a different-port negative.
- Bounded replay `.tmp/phase195_formal_p040_fresh3`/`.tmp/phase195_formal_p040_audit3` is complete: 1,800/1,800 Pair rows are byte/semantic equal to p040 baseline (`0/0` identity delta, zero field drift); issue identities change `11 -> 9`, additions `0`, with only the two target many-to-one rows removed. Visual crops `.tmp/phase195_formal_p040_visual3/crops/PCK0017.png` and `PCK0018.png` show the green KK and small-port mappings exactly overlapping the same source port/endpoint geometry. This slice is accepted at bounded scope and advances to full formal/control regression replay.
- Cycle 195-03 full acceptance: repository gate `1380 passed, 1 skipped`, compileall and diff-check pass. Rules-only formal replay `.tmp/phase195_formal_full_audit3` completes 49/49 and changes issue identities `333 -> 331`: `R-MANY-TO-ONE 17 -> 15`, all other rule counts unchanged, additions zero, and all 331 retained issue objects are equal after excluding sequence-derived `issue_id`. The 28-project/533-DWG control replay `.tmp/phase195_control_533_audit2` remains exactly nine issues with zero additions/removals or retained-object drift.
- The overall goal remains active. Cycle 195-04 begins by clustering the remaining 15 many-to-one, 50 missing-side and 261 low-confidence rows independently; cross-page remains fail-closed unless producer-issued device/scope evidence appears. Selection remains structure-first and must include visual/raw corroboration plus adversarial negatives.
- Cycle 195-04 broad probe batch produced no findings: all three agents failed at the same local proxy/provider boundary with HTTP 504. No source, test, plan or artifact file was modified by those probes. Main-thread aggregation will now reduce the search to exact issue identities before any replacement probes are dispatched.

### Cycle 195-04 UD visual evidence

- Main-thread issue-only clustering narrowed the missing-side family to p041-p044/S0006. The rendered overlay `F:\workspace\XJToolkit\.tmp\phase195_c04_ud_visual_baseline\p041_S0006_UD_expected.png` was inspected against the raw page: eight `1UD10/11/12/13/14/15/16/18` labels sit at the left endpoint of distinct horizontal conductors and visually terminate at numeric labels `719/720/721/722/723/724/201/207`. These are expected number groups, not nearby-text guesses.
- The same overlay shows `1UD20` as a left-only label with no numeric endpoint in its row. It is a required negative control: accepting every `UD` text would create a new left-only review issue. Any producer contract must therefore require a unique, high-confidence opposite numeric candidate with explicit same-row geometry.
- Artifact-authoritative census: 1,265 `^1UD` candidate rows are rejected; the earlier exploratory count of 1,266 was off by one. Distribution is 681 secondary-schematic (577 grid, 104 horizontal), 548 terminal-page horizontal, and 36 component-page rows. Only 36 rows satisfy secondary-schematic + horizontal + compact endpoint geometry, all in p041-p044/S0006. Of those, 32 have the visually confirmed opposite numeric and 4 (`1UD20`, one per project) do not. Existing accepted `XD/YD/LD` wire-logic mappings provide 1,912 structural precedents, but name insertion alone is not authorized.
- Raw-DXF probe found the same eight structures in all four projects. For p041, the `1UD10..18` MTEXT handles are `42DFD/42DFE/42E82/42E83/4827E/4827F/42E00/42E01`; their wire handles are `42E25/42E26/42E6B/42E6C/4827C/4827D/42E27/42E28`. Every label is `(dx,dy)=(2.5,2.4583)` from its left line endpoint and the right numeric is within about `(0.66,1.09)`. `1UD20` is `42E02` on short line `48B43/GW0029`, separated from the right stub and has no right TEXT/MTEXT/ATTRIB.
- Natural negatives add a decisive grammar boundary: `1UD7~8` has right numeric `718` but is a range label and must not be promoted; `1UD5~6` and `1UD3~4` are short rows with no numeric. These 12 rows repeat across p041-p044. All other non-target rows violate page type, orientation, grammar, or compact geometry.
- Selected producer contract: exact `^\d+UD\d+$` grammar; secondary schematic; true horizontal group; at most four drawing units from its endpoint; and an accepted opposite-side numeric candidate whose score is at least the configured high threshold. Multiple numeric peers are allowed only when the top candidate exceeds the runner-up by `0.18`, which keeps the weighted Pair gap above the existing `0.08` ambiguity gate. Missing/low/ambiguous numeric peers become noise before ranking.
- Bounded p041 replay `.tmp/phase195_c04_ud_p041_fresh` is complete for 18/18 DWG with zero incomplete pages. Core `pages/texts/lines/line_groups` tables are exact against the authoritative baseline. Pair rows remain 1,354 with identical `pair_id` keys; after normalizing sequence-derived PairCandidate IDs, semantic/evidence changes are exactly `PW0034/PW0038/PW0044/PW0048/PW0054/PW0057/PW0061/PW0064` and no others.
- The eight current mappings are `1UD18->207`, `1UD16->201`, `1UD15->724`, `1UD14->723`, `1UD13->722`, `1UD12->721`, `1UD11->720`, and `1UD10->719`, all `pass/wire_component_mapping` at confidence `0.9248..0.9268`. `PW0029/GW0029` stays `discard ?->?`; its `1UD20` candidate is `noise_channel/schematic_ud_missing_numeric_peer`.
- Bounded audit `.tmp/phase195_c04_ud_p041_audit` changes `17 -> 9`: exactly the eight target `R-PAIR-MISSING-SIDE` identities are removed, additions are zero, and all nine retained objects are exact after removing derived sequence IDs. PairCandidate IDs after S0006 shift because the eight target groups now create Cartesian candidates; this is bookkeeping-only and every non-target Pair field/evidence value is exact after ID normalization.
- Current-engine visual recheck passes at `F:\workspace\XJToolkit\.tmp\phase195_c04_ud_p041_visual\p041_S0006_UD_current.png`. All eight colored mappings land on the intended same-row left UD label and right numeric endpoint; none crosses rows or terminates on `204/210/213`. `1UD20` remains visibly unpaired and rejected.
- Affected-leaf replays p042/p043/p044 are complete at 18/18, 26/26 and 26/26 DWG with zero incomplete pages. Their Pair counts are 1,357 / 2,258 / 2,266 with exact ID sets. Each normalized semantic/evidence delta is exactly its eight S0006 UD targets, candidate changes are 17 strict UD rows with zero non-UD drift, and `1UD20/GW0029` remains rejected for missing numeric peer.
- Each affected audit changes `17 -> 9`, removes exactly eight `R-PAIR-MISSING-SIDE` identities, adds zero issues and preserves all nine retained issue objects after derived-ID normalization. Across p041-p044 this is exactly 32 intended recoveries and four retained `1UD20` natural negatives.
- Full formal fresh4 completes 49/49 isolated projects and 1,212/1,212 valid DWG with zero invalid or incomplete pages. Pair facts remain 88,265 with exact project-local Pair ID sets. Normalized comparison finds the 32 intended UD semantic changes plus four p037 evidence-only ODA enclosure jitters; p037 values/status/scores/candidates/groups are exact and only rederived enclosure width/height/overlap decimals differ.
- Exactly 32 strict UD candidates are accepted, all in p041-p044/S0006; exactly four `1UD20` rows fail the missing-numeric-peer contract. No other project emits an accepted UD endpoint.
- Full formal audit4 completes 49/49 and changes `331 -> 299`: missing-side `50 -> 18`, while low-confidence `261`, many-to-one `15`, and cross-page `5` remain unchanged. Additions are zero, removals are exactly the 32 p041-p044/S0006 identities, all 299 retained issue objects are exact after derived-ID normalization, and JSON/Parquet/Markdown counts agree for every project.
- Fresh 533-control replay completes 28/28 projects, 533/533 valid DWG, zero invalid/incomplete. Pair facts are `38,825 -> 38,826`: 27 projects are field/evidence exact and `35000_2` adds only `PCK0003/GC0077 2-25DK-3 -> ZD2`, whose raw endpoint `ZD2 &` is the already accepted Cycle 195-01 boundary-ampersand producer. No control project accepts a strict UD endpoint.
- Control audit remains identity/object exact at nine issues (`missing-side 1`, `cross-page 3`, `many-to-one 5`), with zero additions/removals and JSON/Parquet/Markdown parity for all 28 projects. The one producer delta is therefore prior accepted behavior becoming materialized in a fresh control replay, not a Cycle 195-04 regression.
- Final Cycle 195-04 repository gates pass `1383 passed, 1 skipped`, compileall and diff-check. The slice is accepted and persisted in `phase195_cycle_ledger.json`; the overall unattended goal remains active and transitions immediately to Cycle 195-05 evidence clustering over 299 retained fail-closed issues.
- Cycle 195-05 issue-only clustering preserves 15 many-to-one as three structural families: nine table mappings, four strip-two-port components and two strip endpoint bridges. The selected smallest slice is p039/S0025 `PCB0001 3-2ZK-4 -> 3-2n221` versus `P0282`, and `PCB0002 1-2ZK-4 -> 1-2n221` versus `P0179`. Raw physical identity, rule semantics and full-corpus natural negatives must agree before any implementation.
- Cycle 195-05 probes now agree and authorize one narrow rule slice. Raw DXF ties `PCB0001` to INSERT `305DB`, virtual line `305DB:VIRTUAL:2/L6242`, endpoint text handles `31879/3187C`, and `P0282` to backplate block `3A5A9`; `PCB0002` similarly uses INSERT `30543`, `L6154`, handles `31837/3183A`, and backplate row `P0179`. Each pair of facts shares one external endpoint but represents the chain `ZK endpoint -> n endpoint -> NCZ344A row`, not two competing wires.
- The full 88,265-Pair census found seven `strip_two_port_endpoint_bridge` facts: p006 one, p039 two, p047 two and p048 two. All are pass/0.95 and have exact ordinary shadows on endpoint text IDs, coordinates, line group, member line and raw endpoint values; those shadows are discard and `ordinary_pair_eligible=false`. Only p039's two bridges participate in a complete cross-file backplate table many-to-one. The closest p039 ordinary negatives (`PC0047/48/56/73/81`) are review at `0.7754..0.804`, have no component identity, and do not match bridge endpoint IDs, coordinates, line groups or raw values.
- Existing `_is_authoritative_component_table_cross_diagram_endpoint_group` already enforces one component plus one table fact, cross-sheet/file scope, shared endpoint and the same-page outgoing-chain guard, but component identity accepts only KK, ordinary strip and geometry-owned producers. Cycle 195-05 will add a separate endpoint-bridge identity rather than widening ordinary strip identity. The pluginless NCZ backplate row also gets an exact producer identity: row/header/device prefix, text IDs, coordinates, roles and outer Pair fields must agree. Any missing field, low/non-finite confidence, duplicate ID, prefix mismatch, extra Pair or malformed score remains fail-closed.
- Cycle 195-05 implementation adds a separate strict endpoint-bridge identity and a producer-consistent pluginless backplate-row identity. It also closes one narrow audit visibility gap: complete `component_mapping` facts with low or non-finite confidence now emit low-confidence review; the formal and control corpora contain no such current row because every component mapping is exactly `0.95`, while unrelated continuation/semantic/wire producers remain under their existing contracts.
- Bounded p039 raw replay is 30/30 valid with 2,635 Pair rows fully equal to fresh4. The accepted rules audit is `.tmp/phase195_c05_p039_audit3`: `15 -> 13`, zero additions, only `PCB0001/P0282` and `PCB0002/P0179` removed, and all retained objects exact after excluding generated `issue_id`. Pair score restoration in `report/rerun.py::_pair` was required because the historical loader dropped `pair_key`, three scores and `ambiguity_gap` from persisted Pair rows; focused rerun coverage now locks their fidelity.
- Visual recheck passes in `.tmp/phase195_c05_p039_visual2`: S0025 clearly shows both green `ZK-4 -> n221` vertical two-port bridges; S0022/S0023 show the corresponding `n221` on NCZ344A row 23. The first white-background crops are retained as a recoverable visual failure because CAD colors were washed out and labels occluded source text.
- Full formal rules replay `.tmp/phase195_formal_full_audit5` is complete for 49/49 projects and changes `299 -> 297`: low-confidence 261, missing-side 18, many-to-one 13, cross-page 5. Additions are zero; the only removals are the two p039 targets; all retained objects are exact after excluding generated `issue_id`; JSON/Parquet/Markdown parity passes. The reused authoritative fresh4 remains 1,212 valid, 88,265 Pair, zero incomplete.
- Control rules replay `.tmp/phase195_c05_control_533_audit2` is complete for 28/28 projects and remains exact `9 -> 9`; control fresh is 533 valid, 38,826 Pair, zero incomplete. Final repository gate is `1387 passed, 1 skipped`, compileall and diff-check pass. Cycle 195-05 is accepted.
- Cycle 195-06 selects the four remaining component/table shared-endpoint reviews as one structural probe family: p038 `PCM0101/P0243` uses `backplate_virtual_table`; p038 `PCM0042/PTMR0066/PTMR0068` and p039 `PCM0057/PTMR0147/PTMR0148`, `PCM0051/PTMR0424/PTMR0426` use `terminal_header_table`. Prior history treats incomplete KLP/KD ownership as a natural negative, so no change is authorized without new raw physical closure and corpus negatives.
- The exact Cycle 195-06 rule-contract probe rejects all four groups and authorizes no source change. `PCM0101/P0243` has incomplete full-set closure: outgoing sources are `{5KLP6-1, 5n601}` while incoming table sources are `{5KLP6-1, 5n/NTZ302A-1}`; current evidence does not establish that the two `5n` forms identify the same endpoint.
- The `KD26`, `KD1`, and `KD6` groups each contain one strip component plus two distinct terminal-header rows. The terminal rows have different endpoint text IDs and coordinates, occur across sheet/file scope, and have no reciprocal/full-set closure. Adjacency and `KD*`/`KLP*` naming are not physical identity authority.
- Existing fail-closed helpers already encode the necessary boundary: authoritative component identity, authoritative table-row identity, exact two-fact component/table cross-diagram closure, single-sheet reciprocal chains, and single-scope physical terminal clusters. Extending them to these four groups would suppress unresolved conflicts.
- The raw/visual and corpus-census probes exceeded the ten-minute subagent threshold without returning usable evidence and were interrupted. Their silence is not positive evidence. Cycle 195-06 therefore has the evidence-backed disposition `NO_SAFE_CHANGE`; bounded p038/p039 rules replay must prove issue identity remains unchanged before persistence.
- A read-only `rg` search for replay helpers failed to start with Windows `Access denied`; no file changed. Subsequent helper discovery uses native PowerShell `Get-ChildItem`/`Select-String` and does not repeat the same failing invocation.
- Fresh rules-only audit `.tmp/phase195_c06_nochange_audit` completes for p038 and p039 with zero process failures. p038 remains two issues and p039 remains 13 issues. For both projects, `issues.json`, `issues.parquet`, `audit_report.md`, and `runtime_profile.json` are SHA-256 byte-identical to `.tmp/phase195_formal_full_audit5`; therefore every issue identity, field, report row, and runtime payload is unchanged.
- A preliminary PowerShell JSON-shape check failed at parse time because a `foreach` statement block was piped directly. It read no comparison result and changed no file. The corrected script first assigns the loop result to `$rows`; it confirms p038 has 2 issue objects and p039 has 13 with the expected schema.
- A follow-up helper search assumed a repository `scripts/` directory that does not exist and failed read-only. The actual supported audit entrypoint is `python -m dwg_audit.cli run-audit --findings <directory> --output <directory>`.
- Cycle 195-06 final gates pass `1387 passed, 1 skipped`, compileall and diff-check. The no-change disposition is accepted and machine-persisted; the authoritative 297-issue formal audit and nine-issue control audit remain unchanged. Cycle 195-07 immediately starts by structurally clustering the 18 missing-side rows.
- Cycle 195-07 exact issue-only clustering yields four repeated families plus one singleton: p025-p027/S0013 `802 -> ?` (three ordinary wire rows at confidence 0.3085); p040/S0005 two `612 -> ?` rows (0.3299); p041-p044 signal pages eight `? -> 110/419` rows (0.25/0.2844); p041-p044 component pages four `? -> 213` rows (0.3555); and p010/S0018 one component `? -> 201` row (0.3752). This census is the probe boundary; no implementation is selected yet.
- User explicitly cancelled the ten-minute subagent limit. All three Cycle 195-07 probes were allowed to complete; future probes must be awaited unless they explicitly fail or cannot progress.
- Raw/visual probe proves p025-p027 `802 -> ?` uses the bottom edge of the same closed LWPOLYLINE `19F5D` whose top edge carries the real `702 -> 1-4C1D2` mapping. The target edge has no right numeric candidate; it is a component/frame boundary, not an unread number group.
- p040 `612 -> ?` enters centerline device frames `18086/1808C`. The right side contains only `DC output 0~5V/4~20mA-` semantic text rejected as nonnumeric, and the same `612` text is already claimed by a sibling group. Inferring a value would create a false `612 -> 612` mapping.
- p041-p044 `? -> 110/419` rows are repeated closed rectangular LWPOLYLINE boundaries (`26F42`, `26DD2/26E0D`), not open conductors. Their missing sides contain no accepted numeric candidate. The four `? -> 213` rows and singleton p010 `? -> 201` are vertical lines internal to `FJL-25-2A_Mirror`; top `1/2` texts are correctly rejected as `block_internal_pin_number`, not external terminals.
- Code/corpus probe traces every target through ordinary `build_pairs` single-side fallback. Across 88,265 Pair facts, 11,733 ordinary eligible rows contain only 20 single-side rows: the 18 issues plus p040's two related duplicate claims. None of the 18 has an accepted numeric candidate on the missing physical side. Relaxing radius, numeric grammar, block-pin filtering, or logic-row filtering would absorb thousands of natural negatives or producer-owned structured facts.
- Cycle 195-07 therefore has no name-independent, producer-owned, fail-closed correction. Candidate disposition is `NO_SAFE_CHANGE`; retain all 18 reviews and prove exact nine-project audit parity after representative main-thread visual/code verification.
- Main-thread source spot-check confirms the probe path: `page_extractors.py::_extract_pairs_for_route` builds line groups, candidates, then pairs; `pairs.py:54-62` creates review only when one accepted side is absent; `pairs.py:253-266` admits only accepted numeric/logic/semantic channels. `candidates.py:220-231` rejects nonnumeric and out-of-row logic text, while `candidates.py:305-316` rejects virtual block internal pin numbers. These guards are semantically required by the observed natural negatives.
- The first representative Pair JSON extraction repeated the invalid PowerShell `foreach | ConvertTo-Json` form and failed during parsing. It changed no files; the corrected form assigns loop output to `$rows` before serialization.
- The corrected JSON query showed `findings.json` does not embed a `pairs` array, so it returned six empty matches. This is a reader-layer mistake, not missing recognition output. Direct structured reads of `pairs.parquet` then verified seven representative Pair objects, including exact values, confidence, line coordinates, selected raw text, score breakdown, and missing-side rationale.
- The first Cycle 195-07 render attempt created an empty output directory and failed before drawing because this ezdxf version lacks `RenderContext.set_background_color`. The retry verified the directory was empty, removed only the unsupported call, and generated five images under `.tmp/phase195_c07_visual`.
- Main-thread visual review passes all five representative crops. `p025_802_closed_frame.png` shows the highlighted target as a closed rectangular frame edge; `p040_612_device_inputs.png` shows both target lines entering device boxes whose right-side content is DC-output description text; `p041_110_419_closed_frames.png` shows both target lines closing relay/signal rectangles; `p041_213_internal_pin.png` and `p010_201_internal_pin.png` show the highlighted line directly between internal pin circles `1` and `2`, with the actual external endpoint text below. None contains a visually unread but expected missing endpoint number.
- Nine-project bounded rules replay `.tmp/phase195_c07_nochange_audit` completes with zero process failures. p010 remains 2 issues, p025-p027 remain 1 each, p040 remains 9, and p041-p044 remain 9 each. All 36 `issues.json`/`issues.parquet`/`audit_report.md`/`runtime_profile.json` outputs are SHA-256 byte-identical to formal audit5; all 18 missing-side objects remain exact with zero additions or removals.
- Cycle 195-07 final gates pass `1387 passed, 1 skipped`, compileall and diff-check. The cycle is persisted as `NO_SAFE_CHANGE`, retaining all 18 reviews. Cycle 195-08 immediately begins structural clustering of the 261 low-confidence issues; lowering confidence thresholds is explicitly outside the candidate space.
- Cycle 195-08 issue-only census confirms 261 low-confidence reviews. The dominant component-grid family is 207 rows in p016-p021: 105 exact `2 -> 28` rows at confidence 0.7816 and approximately 102 `xxxx -> 27` rows dominated by confidence 0.7726. Smaller bounded families are p003 four component rows (`1 -> 401/402/405/818`), p010/p011/p015 six sparse terminal rows, p039 nine rows, and p040-p044 six rows per project.
- Audit issue objects do not embed producer Pair evidence, so producer/source/channel fields were null in the issue-only view. Cycle 195-08 probes must join exact issue IDs back to `pairs.parquet`, candidates, raw DXF and visual regions. No threshold change or implementation is authorized from issue counts alone.
- Cycle 195-08 terminal-table subcluster was checked against the authoritative Pair and table artifacts. The six target ordinary Pairs are `p010/S0021/F0021/PT0404` (`104 -> 12`, `T4636/T4635`), `p011/S0021/F0021/PT0145|PT0149|PT0153|PT0157` (`13 -> 514`, `12 -> 513`, `11 -> 512`, `10 -> 515`, text IDs `T4088/T4089`, `T4085/T4086`, `T4082/T4083`, `T4079/T4080`), and `p015/S0022/F0022/PT0002` (`1 -> 913`, `T4299/T4300`).
- Structured lookup found zero target-text-ID matches in the corresponding `table_extraction_summary.mappings` arrays: p010 has 495 mapping rows, p011 682, and p015 666. No generated table Pair ID exists for any target. Same-filename coordinate checks also fail closed: p010's only mapped row near y=266 is `1I17D` at x=66/89, p011's nearby mappings are at other columns (x=151/158, x=311/334/341) rather than the target x=243/251, and p015 has no mapping at the target y=46 row.
- The ordinary candidate geometry is real same-row terminal-grid evidence, but the table producer did not emit header/row/endpoint ownership for these rows. Reusing neighboring table rows, row numbers, filenames, or normalized values would be name/position inference rather than producer-owned evidence. Existing ordinary Pair review status therefore remains fail-closed. Verdict: `NO_SAFE_CHANGE`; continue Cycle 195-08 on the p016-p021 component-grid family.

## Cycle 195-08 p003 low-confidence disposition

- The selected p003/S0012 family contains four ordinary review pairs: `PC0029 1->401`, `PC0031 1->402`, `PC0033 1->405`, and `PC0037 1->818`. All are `status=review`, `confidence=0.8099`, `ambiguity_gap=null`, with `left_score=0.9659`, `right_score=0.7449`, and `wire_score=0.4`.
- The selected line groups are vertical 15-unit boundary lines (`GC0029`, `GC0031`, `GC0033`, `GC0037`), not electrical wires. The left `1` texts are virtual internal pins from `FJL-25-2A Y4_Mirror` or `FJL-25-2A R4_Mirror`; the right `8n401/8n402/8n405/8n818` texts are derived numeric labels with `distance_y=6.5`. The 0.8099 score is therefore endpoint/line geometry weakness, not candidate ambiguity.
- Raw DXF rendering classifies each target as a narrow vertical capsule: internal pins `1` and `2` are inside the symbol, boundary lines run from y=217.5 to y=202.5, and external labels sit outside. The selected `1->401/402/405/818` mappings are internal-pin mispairs, not legitimate numbering groups.
- The candidate guard only rejects exact configured block families (plus the existing `_MIRROR` form). `FJL-25-2A Y4_Mirror` and `FJL-25-2A R4_Mirror` escape that matcher. The component producer has the same exact-name limitation, and overlapping external labels (`8nB01`/`8n401`, etc.) prevent a producer extension from being fail-closed without an explicit duplicate-anchor ownership contract.
- No candidate-only fix is safe: broadening internal-pin rejection would hide valid endpoint relationships unless the component producer is extended at the same time, while producer extension without duplicate-anchor disambiguation can select the wrong external label. Disposition is `NO_SAFE_CHANGE`.
- Fresh rules-only replay `.tmp/phase195_c08_p003_nochange_audit/p003` produced outputs byte-identical to `.tmp/phase195_formal_full_audit5/p003` for `issues.json`, `issues.parquet`, `audit_report.md`, and `runtime_profile.json`; issue count remains four with zero additions/removals or object drift.
- Final gates remain green: `1387 passed, 1 skipped`, compileall pass, and diff-check pass. The remaining formal audit is unchanged at 297 issues: 261 low-confidence, 18 missing-side, 13 many-to-one, and 5 cross-page.

## Cycle 195-08 p016-p021 component-grid disposition

- Exact target count is 207: 105 `2 -> 28` rows and 102 rows whose left value is exactly four digits and right value is `27`. Every target is an ordinary review Pair on one vertical 70-unit `LWPOLYLINE`, layer `BORDER`, block `TSB-14`, with a virtual handle ending in `:VIRTUAL:56:2` or `:VIRTUAL:56:6`.
- High-contrast raw-DXF review shows the selected line on a rounded rectangular terminal-grid border with circular pin artwork and no conductor continuation. Representative evidence includes p016 `PC0099/GC0099/T4232/T4206/L6080` (`2 -> 28`) and `PC0097/GC0097/T4458/T4213` (`2213 -> 27`), plus p020 `PC0130/GC0130/T4992/T4966/L7453`.
- Natural negatives prove values cannot authorize the rule: p021 `PC0137 1 -> 27` has the same frame provenance, while p017 `PT0358 104 -> 12` and p021 `PT0002 1 -> 913` are real horizontal layer-0 LINE rows. Natural positive component mappings in all six projects are separate `PCM*` pass/0.95 facts with complete body, port, external endpoint and supporting-line evidence.
- The current model has no producer-issued `component_grid_boundary` or closed rounded-frame ownership field. The only exact selector available is definition name plus internal virtual index plus dimensions. That is a template-specific fingerprint, not the required name-independent fail-closed contract. The attempted structural shadow was therefore removed before any fresh replay.
- Rules-only replay `.tmp/phase195_c08_grid_nochange_audit` is byte-identical to `.tmp/phase195_formal_full_audit5` for all four artifacts in p016-p021 (24/24 files). Disposition: `NO_SAFE_CHANGE`; retain the 207 reviews until extraction persists generic closed-grid producer authority.

## Cycle 195-09 p039-p044 repeated low-confidence disposition

- The selected 39-row cluster is exactly: p039 nine rows, p040 six rows, and p041-p044 six rows each. All are `R-PAIR-LOW-CONFIDENCE`, `pair_kind=ordinary_pair`, `status=open` in audit5, and remain review after replay.
- Raw/DXF visual evidence confirms two distinct families. The p039/p040 `PC*` rows are vertical 15-unit `LINE` geometry on `FJL-25-2A_Mirror:VIRTUAL:2`, with derived numeric text endpoints but no body/port/external-endpoint/full-set `component_mapping` evidence. The p039/p040 `PW*` rows are real `CONNECT` lines; p039 has distinct DIM endpoints `415->416` and `413->414`, while p040 and p041-p044 include one physical DIM text reused as both candidate sides. These are not covered by an existing producer-owned shadow.
- Code-contract review confirms `page_extractors.py` only consumes ordinary pairs when a component producer emits complete ownership, `communication_media` shadows require nonempty page media evidence, and `rules.py` emits low-confidence issues for otherwise eligible ordinary pairs. The component producer contract in `component_diagrams.py` requires body, ports, external endpoints and supporting lines; none of the nine `PC*` targets satisfy it.
- Full-corpus natural census found 749 ordinary same-text pairs: 712 already discard and 37 remain review, with zero pass. The review population includes grid and horizontal natural counterparts, so generic same-text suppression would hide missing-side/ambiguous cases. No name-independent `self_text_two_endpoint` contract is justified in this cycle.
- The completed third probe independently confirms the broader corpus boundary: 32,548 ordinary pairs contain 2,639 reviews and zero passes; only 16 natural `CONNECT` rail shadows have the required interior-drop producer evidence, while the target rows have none. Existing multi-endpoint network shadowing covers one distinct producer-owned row with at least three authoritative terminal-port bindings; none of the 39 targets has that ownership. Relevant contracts are `page_extractors.py:1049-1137`, `page_extractors.py:1145-1330`, `page_extractors.py:2009-2015`, and `rules.py:380-408`.
- Bounded rules replay `.tmp/phase195_c09_nochange_audit` covers p039-p044. All 24 `issues.json`, `issues.parquet`, `audit_report.md`, and `runtime_profile.json` files are SHA-256 byte-identical to `.tmp/phase195_formal_full_audit5`. Counts remain p039=13, p040-p044=9 each; low-confidence counts remain 9/6/6/6/6/6, with zero additions/removals and zero retained-object drift.
- Disposition: `NO_SAFE_CHANGE`. No recognition source or test file changed. The next bounded cluster is the five residual p017/p021 rows: `PT0358 104->12`, `PC0088 2001->2012`, `PT0002 1->913`, `PC0106 2001->2012`, and `PC0137 1->27`.

## Cycle 195-10 p017/p021 residual low-confidence disposition

- The five selected rows are two horizontal terminal-table rows (`p017/PT0358 104->12`, `p021/PT0002 1->913`) and three vertical `BORDER` rows (`p017/PC0088 2001->2012`, `p021/PC0106 2001->2012`, `p021/PC0137 1->27`). The horizontal groups are 75-unit layer-0 rows with ordinary terminal candidates but no table producer ownership in their Pair evidence; the vertical groups are 70-unit BORDER rows with alternatives and/or `TSB-14` block-owned text.
- High-contrast crops `.tmp/phase195_c10_visual/p017_PT0358.png`, `p021_PT0002.png`, `p017_PC0088.png`, and `p021_PC0137.png` show table/frame geometry rather than a uniquely readable conductor. The p017/p021 vertical rows are the same component-grid family already rejected in Cycle 195-08; the two horizontal rows match the earlier terminal-table family whose target text IDs were absent from table-extraction ownership.
- Rules-only bounded replay `.tmp/phase195_c10_nochange_audit` covers p017 and p021. All eight `issues.json`, `issues.parquet`, `audit_report.md`, and `runtime_profile.json` files are SHA-256 byte-identical to formal audit5. Both projects remain at nine issues; all five target low-confidence rows remain open review, with zero additions/removals and zero retained-object drift.
- Disposition: `NO_SAFE_CHANGE`. No recognition source or test file changed. The next active cluster is the five formal cross-page conflicts: `P0030`, `P0116`, `P0018`, `P0155`, and `P0107`, all involving repeated `1n/NCK305-*` to `1C1D1/1C2D1` endpoint families.

## Cycle 195-12 many-to-one structural disposition

- The selected residual family contains all 13 formal `R-MANY-TO-ONE` reviews: p006 `PTMR0337/8ID-1 -> 8n801`; p014 `PTMR0130`, `PTMR0139`, `PTMR0198`; p015 and p021 `1C9D-11 -> GND`; p038 `PCM0042` and `PCM0101`; p039 `PCM0051`, `PCM0057`, `PTMR0136`, `PTMR0140`; and p040 `PTMR0225`.
- The producer-backed two-edge cross-diagram contract at `rules.py:2185-2322` is safe only for exactly one component Mapping plus one table Mapping with strict identity, scope, and reciprocal closure. It does not apply to the three-edge KD26/KD1/KD6 groups, table-only 1-2GD3/3-2GD3 groups, or the GND/1ZK-3 table-only groups. The two-edge p038 `5KLP8-1` group also fails because same-sheet component outtargets `{5KLP6-1, 5n601}` do not close against the backplate incoming identity `5n/NTZ302A-1`.
- Raw and visual evidence is consistent with retaining review. p006 shows two distinct `8n801` TEXT instances at separate x coordinates in one table row; p014's repeated `1C14D6/1C17D6/1C19D6` values occupy different table cells/pages; p015/p021 `GND` values use distinct sheets and text coordinates; p040 `1ZK-3` is repeated in separate table rows and text identities. The p038/p039 component/table crops already retained in `.tmp/phase195_c04_many_exact_visual` show their separate component bodies, ports and table rows without a closed union contract.
- Positive tests at `tests/unit/test_pairs_and_rules.py:944-971` accept only the strict two-edge producer contract. Adversarial tests at `:927-941`, `:974-1005`, `:1008+`, and `:7097-7165` keep mismatched, review-status, low-confidence, same-sheet, and terminal-only groups visible. No name-independent generalization is justified.
- Bounded replay `.tmp/phase195_c12_nochange_audit` covers p006, p014, p015, p021, p038, p039 and p040. All 28 persisted JSON/Parquet/Markdown/runtime files are SHA-256 byte-identical to `.tmp/phase195_formal_full_audit5`; issue counts are `1/4/2/36/2/13/9` respectively and the 13 target many-to-one reviews remain unchanged. No additions, removals or retained-object drift occurred.
- Visual refs: `.tmp/phase195_c12_visual/p006_PTMR0337.png`, `p014_PTMR0130.png`, `p014_PTMR0139.png`, `p014_PTMR0198.png`, `p015_PTMR0098.png`, `p021_PTMR0098.png`, `p040_PTMR0225.png`, and `p040_PTMR0263.png`.
- Disposition: `NO_SAFE_CHANGE`. No recognition source or test file changed. All 13 many-to-one reviews remain fail-closed; the next loop state returns to corpus-wide discovery of unrecognized or under-recognized number groups.

## Cycle 195-11 cross-page scope disposition

- The selected cluster contains the five formal `R-CROSS-PAGE-CONFLICT` issues: p008 `P0030`, p009 `P0116`, p012 `P0018`, p014 `P0155`, and p020 `P0107`. Each primary Pair is a complete `backplate_virtual_table` mapping with `status=pass`, confidence `0.95`, `composite_device_instance=1n`, header `NCK305`, column `c0`, and a synthesized logical endpoint `1n/NCK305-{row}@c0`.
- Raw DXF and visual review show that each conflict joins page-1 and page-2 backplate instances: source blocks differ as `WMH-800-D1C-02-1` versus `...-02-2` (p012 uses `...-01-1` versus `...-01-2`), and the physical right terminals differ (`1C1D1` versus `1C17D1`, or `1C2D1` versus `1C18D1`). The pages use different sheet/file scopes and unique header/row/right text IDs and coordinates; these are not duplicate right values in one table instance.
- Producer and rule contracts confirm that `table_extractor.py:1613-1645,1872-1896` emits physical table evidence but no independent device/plugin scope for these rows. `rules.py:456-526,1160-1228` therefore keeps reused-header groups in `backplate_table_scope_review`; the only existing suppression requires distinct validated raw headers per source block. Existing tests at `tests/unit/test_pairs_and_rules.py:5753-5865` establish the positive distinct-header boundary and the reused-header negative.
- No safe name-independent implementation exists. Treating page/file IDs, block suffixes, repeated `NCK305`, text IDs/coordinates, or `c0` as independent device scope could suppress genuine same-device cross-page conflicts. All five reviews remain fail-closed.
- Bounded rules replay `.tmp/phase195_c11_nochange_audit` covers p008, p009, p012, p014, and p020. The four persisted artifact types (`issues.json`, `issues.parquet`, `audit_report.md`, `runtime_profile.json`) are SHA-256 byte-identical to `.tmp/phase195_formal_full_audit5` for all 20 files. Issue counts and retained objects are unchanged; no additions, removals, or drift occurred.
- Disposition: `NO_SAFE_CHANGE`. No recognition source or test file changed. The next active cluster is the remaining 13 `R-MANY-TO-ONE` issues, to be structurally clustered by producer ownership and endpoint cardinality.

## Phase 196 post-completion adversarial audit (2026-08-31)

- Scope is the completed Phase 195 cycle 195-14 implementation and cleanup recorded in `progress.md`, especially commit `f51b26c`, the TSB terminal-strip lattice recognizer, long-chain audit record, full replay artifacts, and the current source tree.
- The referenced Codex task did not itself yield a reliable final response: `read_thread` exposed repeated 429/provider failures and the local JSONL ends with a provider error. Its visible context is useful for intent and prior probes, but completion claims must be verified locally.
- Existing planning files are user/project history and were preserved; this section is the only new audit context. No business-source edit is authorized by the audit request.
- The six requested parallel probes produced no usable output: each remained `running` through repeated 60-second waits for more than ten minutes, then was closed. This is a verification-coverage gap, not evidence that the code is clean.
- Main-thread baseline: `HEAD=7ab32ad`; implementation commit `f51b26c` adds 1,328 lines across the terminal-strip/series-chain path and the following commit records the replay baseline. Only the three planning files are currently modified by this audit.
- The referenced session confirms the user acceptance intent: recognize the terminal-strip rows through structure rather than a `TSB` block-name gate, with the user-provided `TF` row mappings as the semantic ground truth.
- Candidate evidence inconsistency: `.tmp/tsb_full_replay/progress.jsonl` contains an `audit_failed:2` event followed by `ok` for every project `p000`-`p048`. This must be reconciled against the driver, per-project `issues.*`/runtime artifacts, and exit status before treating the full-replay claim as clean.
- Confirmed replay summary mismatch: `.tmp/tsb_full_replay/summary.json` reports `totals.after=88`, while `phase195_cycle_ledger.json` and the Phase 195 narrative claim `297 -> 87`. The final progress row for `p042` is `issues_before=9`, `issues_after=10`, so the transient conversion failure was not actually removed from the stored full-replay total.
- `.tmp/tsb_full_replay/progress.jsonl` groups to 69 `ok`, 27 `audit_failed:2`, and 39 missing-path `error:*` intermediate events; the driver must establish whether these are intentionally discarded attempts or evidence of a non-clean replay. The summary's empty `failures` array alone is insufficient.
- The replay driver is non-idempotent: `.tmp/full_replay_driver.py:54-55,71-72,153-161` appends to an existing `progress.jsonl`, reuses an existing output project when `pairs.parquet` exists, and overwrites `summary.json` without a run identifier or manifest of the exact attempt. The current progress stream contains duplicate final rows for `p004`-`p021` and stale failed/error attempts.
- Current artifact drift is confirmed: `.tmp/tsb_full_replay/summary.json` stores `after=88` and the historical `p042` progress row stores `9 -> 10`, but the current `.tmp/tsb_full_replay/p042/1WFB-821E_1WFB-822E/audit/issues.json` contains 9 records. The claimed corpus total cannot be accepted without a fresh isolated replay or an artifact-indexed recomputation.
- Confirmed P1 code defect: `src/dwg_audit/audit/series_chains.py:89-121` accepts a strip pair and a component pair from the same `sheet_id/file_id` as a cross-view series chain because it checks only producer submodes, values, status, and confidence. `src/dwg_audit/audit/rules.py:845-856` calls this before emitting `R-MANY-TO-ONE`. A minimal in-memory fixture with both pairs on `SAME/F-SAME` returned `same_sheet_chain=True` and `same_sheet_rules=[]`; duplicate physical text IDs also returned `True`. This can hide a real same-page multi-entry conflict.
- Confirmed P2 artifact-integrity defect: `src/dwg_audit/audit/series_chains.py:141-149` records every passing `component_mapping` continuation whose `left_value` equals the junction, without requiring `_PORT_OWNER_SUBMODES` or `evidence.source == component_mapping` with an allowed submode. A fixture added with `component_prefixed_signal_circuit` was serialized as a continuation, so the chain record can claim unrelated producer evidence.
- Current artifact-indexed recomputation over `.tmp/tsb_full_replay/p000`-`p048` found 49 projects, 87 issues, 40 chains, and 5,189 component mappings. This matches the intended 297 -> 87 arithmetic but not the stored `summary.json` (`after=88`), proving the summary is stale relative to the current files.
- Confirmed P2 integration defect: `src/dwg_audit/audit/component_diagrams.py:425-427` marks a structurally detected strip's flank groups consumed even when every row is unlabeled/single-sided and `terminal_strip_pairs` is empty. But `src/dwg_audit/audit/page_extractors.py:266-269` applies `consumed_group_ids` only inside `if component_pairs:`. Therefore a strip with no valid mapping leaves ordinary border pairs untouched, despite the detector having identified the geometry that should shadow them. No route-level test currently proves this empty-component-pair boundary.
- The current unit suite covers direct lattice positive/negative fixtures and chain predicates, but the missing cases above are specifically same-sheet chain identity, unrelated continuation producers, and empty-mapping consumed-group propagation.
- Confirmed P2 route reproduction: with a structurally detected strip returning zero valid row mappings but consumed `GCL/GCR`, a monkeypatched `extract_component_pairs()` leaves an ordinary `GCL` pair at `status=review` with empty evidence. This follows directly from `page_extractors.py:244-269` and the detector contract at `component_diagrams.py:425-427`; the consumed groups are only applied when `component_pairs` is truthy.
- Verification results: focused component/chain tests `57 passed`; full suite `1414 passed, 1 skipped`; `python -m compileall -q src` and `git diff --check f51b26c^ HEAD` passed. These are baseline health checks, not proof against the three hostile fixtures.
- Main-thread corpus check over the current `.tmp/tsb_full_replay` artifacts found 49 `series_chains.json` files, 40 chains, 90 chain member rows, and zero continuation rows. Every current chain spans at least two sheet/file scopes, so the same-scope defect below is not a current-corpus hit; it remains a directly reproducible code-path risk.
- Confirmed P1 boundary defect: `is_cross_view_terminal_series_chain()` has no distinct `sheet_id`/`file_id`, text identity, or physical-coordinate requirement. A same-sheet/same-file strip + KK pair returns `True`; the same fixture reaches `rules.py:852-856` and suppresses `R-MANY-TO-ONE`. Duplicating physical text IDs still returns `True`. The unit helper itself defaults `file_id=sheet_id` (`tests/unit/test_series_chains.py:12-44`), but no negative test asserts same-scope rejection.
- Confirmed P2 continuation defect: `build_terminal_series_chains()` filters normal members through `_is_chain_member()` (`series_chains.py:59-72`) but its continuation comprehension (`:141-149`) checks only `pair_kind`, `status`, finite confidence, threshold, and `left_value`. A `component_prefixed_signal_circuit` pair was serialized as a continuation even though it is absent from `_PORT_OWNER_SUBMODES`; current corpus has `continuation_count=0`, so this is latent artifact contamination rather than a current hit.
- Confirmed P2 route defect: `_detect_terminal_strips()` can create a strip and `extract_terminal_strip_lattice_pairs()` can return consumed flank groups with zero valid paired rows (`component_diagrams.py:425-446`). The ComponentDiagramExtractor route applies those groups only inside `if component_pairs:` (`page_extractors.py:266-269`). A monkeypatched route fixture leaves an ordinary `GCL` border pair at `review` despite consumed `GCL/GCR`; no route-level regression test covers this.
- Current full replay recomputation is internally consistent at the artifact level only: 49 issue artifacts sum to 87 records, 49 chain artifacts sum to 40 chains, and component-mapping rows sum to 5,189 (`terminal_strip_lattice=425`, other component submodes=4,764). The stored `.tmp/tsb_full_replay/summary.json` says `after=88` and calls all 5,189 component mappings `strip_pairs`, so both the total and metric label are stale/misleading.
- Replay reproducibility is materially broken: `.tmp/full_replay_driver.py:17` still points at `.tmp/phase195_formal_full_fresh4`, which currently does not exist; `main()` would enumerate zero baseline manifests and overwrite `summary.json` (`:54-55,153-161,218-219`). It also appends to `progress.jsonl` and reuses project output directories without a run ID or manifest (`:54-72`). The current stream has 134 rows for 49 pids: 69 `ok`, 27 `audit_failed`, and 38 missing-path errors, with repeated final rows. The clean 49-project claim cannot be reconstructed from this stream alone.
- Current residual `R-MANY-TO-ONE` artifacts total 13 and none is a current chain member; current chain records are confined to p016-p021. Thus the chain implementation did not hide an observed residual many-to-one issue in this artifact set beyond the intentionally delegated 40 junctions; the same-scope and continuation findings remain hostile-fixture findings.

## Cycle 195-13 orphan-number census and component-port boundary

- The corpus-wide discovery pass over `.tmp/phase195_formal_full_fresh4` found `89,305` numeric-like TEXT rows. After excluding Pair endpoint IDs and candidate-linked text IDs, `46,047` remain orphan rows: `406` are referenced only inside Pair evidence and `45,641` have no Pair/candidate/evidence reference. This is a discovery signal, not permission to promote numeric text.
- The smallest structure-backed positive cluster is p039/S0025/F0025, route `ComponentDiagramExtractor`, block `FJL-25-2A_Mirror`: T4480=`1` and T4479=`2` are internal ports referenced in `PCB0001` evidence, with external endpoints T4619=`3-2ZK-4` and T4620=`3-2n221`, support group GC0055 and line L6242. The visual composite `.tmp/phase195_c04_many_exact_visual/p039_PCB0001_3-2n221_composite.png` shows the 1/2 circles inside the component and the external endpoint labels outside.
- The adversarial twin in the same block and geometry (T4435=`1`, T4434=`2`, GC0056/L6143) has no valid external endpoints and no component Pair; both candidates are rejected as `block_internal_pin_number`. Corpus-wide, only seven complete `strip_two_port_endpoint_bridge` facts exist, while 747 ordinary internal-pin pairs are discarded and 3,402 FJL virtual numeric texts are present. Geometry or numeric value alone is therefore insufficient.
- The producer contract is explicit: `src/dwg_audit/audit/component_diagrams.py:87-204` requires body, paired ports, two grammar-valid external endpoints, matching prefix and support geometry; `:1168-1318` stores port IDs in evidence while Pair left/right IDs remain external endpoints. `src/dwg_audit/audit/candidates.py:305-316,1851-1875` and `src/dwg_audit/audit/pairs.py:46-80,434-482` keep internal virtual pins out of ordinary Pair values. Existing positive/negative tests at `tests/unit/test_component_diagrams.py:118-142,463-519` cover this boundary.
- A bounded p039 rules replay under `.tmp/phase195_c13_nochange_audit/p039` produced the same four audit artifacts as `.tmp/phase195_formal_full_audit5/p039`: all 4 SHA-256 pairs are exact; issue count remains 13 with zero additions, removals or retained-object drift. Full gates remain `1387 passed, 1 skipped`, compileall pass and diff-check pass.
- Disposition: `NO_SAFE_CHANGE`. Do not resurrect orphan component ports as ordinary numbered groups. The loop advances to repeated grid stubs with alphanumeric device designators (`p012/p013`), where the next probes must distinguish genuine missing numbers from intentional nonnumeric endpoints and terminal continuation rows.
