...
- Final review reproduced Windows blocking when one thread closes an FD while another is in `os.read`. Capture descriptors are now nonblocking and poll a stop event; only the reader thread closes its own FD/stream. A held-open writer test proves shutdown remains bounded.
- The frozen post-build smoke now has a 15-second `Process.WaitForExit` bound and kills the smoke process on timeout, so a broken bootloader cannot hang release packaging.
